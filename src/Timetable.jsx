...
const T = {
  ...
  green:'#10b981', purple:'#8b5cf6', purpleLt:'#f5f3ff',
...
function AdminPinModal({ onSuccess, onClose }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(10,22,40,0.75)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:'white', borderRadius:16, padding:36, width:'100%', maxWidth:360, boxShadow:'0 24px 80px rgba(0,0,0,0.4)', textAlign:'center' }}>
...
function AdminSetupPanel({ staffList, entries, onRefresh, showToast }) {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${count} entries${delBatch?` for ${delBatch}`:''}${delDay?` on ${delDay}`:''}?`)) return
  ...
  const loadPreview = () => {
    setPreviewData(entries.filter(e=>(!previewDay||e.day_name===previewDay)&&(!previewBatch||e.class_name===previewBatch)))
  }
    ...
    <div>
      ...
      {tab==='batch' && (
        <div style={{ background:'white', borderRadius:14, padding:24, boxShadow:'0 2px 12px rgba(0,0,0,.07)', border:`1px solid ${T.border}` }}>
          ...
          <div style={{ fontSize:12, color:T.muted, marginBottom:20 }}>Set batch + day once, then add all periods at once. Teachers are pulled from staff profiles.</div>
      ...
      {tab==='copy' && (
        <div style={{ background:'white', borderRadius:14, padding:24, boxShadow:'0 2px 12px rgba(0,0,0,.07)', border:`1px solid ${T.border}` }}>
          ...
          <div style={{ marginTop:20, padding:'12px 16px', background:'#fffbeb', borderRadius:8, border:`1px solid ${T.gold}`, fontSize:12, color:T.goldDim, fontWeight:600, marginBottom:20 }}>
            ⚠️ Warning: All existing entries for the selected target days will be deleted and replaced.
          </div>
      ...
      {tab==='preview' && (
        <div style={{ background:'white', borderRadius:14, padding:24, boxShadow:'0 2px 12px rgba(0,0,0,.07)', border:`1px solid ${T.border}` }}>
          <div style={{ fontSize:15, fontWeight:800, color:T.navy, marginBottom:4 }}>👁 Preview & Verify</div>
          <div style={{ fontSize:12, color:T.muted, marginBottom:20 }}>Filter and review entries before confirming setup</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:12, marginBottom:16 }}>
            <div><label style={S.lbl}>Filter by Day</label><Sel value={previewDay} onChange={e=>setPreviewDay(e.target.value)}><option value="">All Days</option>{DAYS.map(d=><option key={d} value={d}>{d}</option>)}</Sel></div>
            <div><label style={S.lbl}>Filter by Batch</label><Sel value={previewBatch} onChange={e=>setPreviewBatch(e.target.value)}><option value="">All Batches</option>{uniqueClasses.map(c=><option key={c} value={c}>{c}</option>)}</Sel></div>
            <div style={{ display:'flex', alignItems:'flex-end' }}><button onClick={loadPreview} style={{ ...S.btn(T.navy), padding:'9px 20px' }}>Load Preview</button></div>
          </div>
          {previewData.length>0&&(
            <div style={{ overflowX:'auto' }}>
              <div style={{ marginBottom:8, fontSize:12, color:T.muted, fontWeight:600 }}>{previewData.length} entries</div>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                <thead>
                  <tr style={{ background:T.navy }}>
                    {['Day','Period','Batch','Section','Subject','Teacher','Room'].map(h=>(
                      <th key={h} style={{ padding:'9px 12px', textAlign:'left', color:'white', fontWeight:700, fontSize:11, textTransform:'uppercase', letterSpacing:'.06em' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {previewData.sort((a,b)=>a.day_name.localeCompare(b.day_name)||a.period_name.localeCompare(b.period_name)).map((r,i)=>{
                    const bs = getBatchStyle(r.class_name)
                    return (
                      <tr key={r.id} style={{ background:i%2===0?'white':T.surface, borderBottom:`1px solid ${T.border}` }}>
                        <td style={{ padding:'8px 12px', color:T.ink, fontWeight:600 }}>{r.day_name}</td>
                        <td style={{ padding:'8px 12px', color:T.muted, fontFamily:'monospace', fontSize:11 }}>{r.period_name}</td>
                        <td style={{ padding:'8px 12px' }}><span style={{ background:bs.bg, color:bs.text, border:`1px solid ${bs.border}`, padding:'2px 8px', borderRadius:999, fontSize:11, fontWeight:700 }}>{r.class_name}</span></td>
                        <td style={{ padding:'8px 12px', color:T.muted }}>{r.section||'—'}</td>
                        <td style={{ padding:'8px 12px', color:T.ink, fontWeight:600 }}>{r.subject_name}</td>
                        <td style={{ padding:'8px 12px', color:T.muted }}>{r.teacher_name||'—'}</td>
                        <td style={{ padding:'8px 12px', color:T.muted }}>{r.room_name||'—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
          ...
          {previewData.length===0&&<div style={{ padding:'40px', textAlign:'center', color:T.muted, fontSize:13 }}>Click "Load Preview" to view entries</div>}
        </div>
      )}
      ...
      {tab==='delete' && (
        <div style={{ background:'white', borderRadius:14, padding:24, boxShadow:'0 2px 12px rgba(0,0,0,.07)', border:`1.5px solid ${T.red}44` }}>
          ...
          <div style={{ fontSize:12, color:T.muted, marginBottom:20 }}>Delete all entries for a specific day or batch. Use with caution — this cannot be undone.</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, maxWidth:500, marginBottom:16 }}>
            <div><label style={S.lbl}>Delete by Day</label><Sel value={delDay} onChange={e=>setDelDay(e.target.value)}><option value="">All Days</option>{DAYS.map(d=><option key={d} value={d}>{d} ({entries.filter(e=>e.day_name===d).length} entries)</option>)}</Sel></div>
...
export default function Timetable() {
  ...
  const dailyViewData = useMemo(()=>{
    ...
    return days.map(day=>({ day, periods:[...new Set(entries.filter(e=>e.day_name===day).map(e=>e.period_name).filter(Boolean))].sort(), dayEntries:entries.filter(e=>e.day_name===day), allBatches }))
  },[entries,dayFilter])
  ...
  const viewTabs = [
    ...
    {id:'admin',   label:'⚙️ Admin Setup'},
  ]

  return (
    <div style={{ padding:'24px 28px', fontFamily:"'DM Sans','Segoe UI',sans-serif", background:'#f0f4f8', minHeight:'100vh', color:T.ink }}>

      {toast&&<div style={{ position:'fixed', top:20, right:20, zIndex:9999, padding:'12px 20px', borderRadius:10, background:toast.type==='error'?T.red:T.navy, color:'white', fontWeight:600, fontSize:13, boxShadow:'0 8px 32px rgba(0,0,0,0.2)' }}>{toast.type==='error'?'⚠️':'✓'} {toast.msg}</div>}
          ...
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
              ...
              <div>
                ...
                <div style={{ fontSize:12, opacity:.6, marginTop:1 }}>GNSI · Academic Schedule · {entries.length} entries · {staffList.length} staff loaded</div>
              </div>
            </div>
            <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:4 }}>
              ...
              <span style={{ background:'rgba(255,255,255,0.1)', color:'white', padding:'3px 10px', borderRadius:999, fontSize:11, fontWeight:600 }}>{todayEntries.length} classes today</span>
              ...
              {adminUnlocked&&<span style={{ background:'rgba(240,165,0,0.3)', color:T.gold, padding:'3px 10px', borderRadius:999, fontSize:11, fontWeight:700 }}>🔓 Admin Active</span>}
            </div>
          </div>
        ...
        <div style={{ background:T.redLt, border:`1px solid ${T.red}44`, borderRadius:12, padding:16, marginBottom:20 }}>
          ...
          {conflicts.map((c,i)=>(
            <div key={i} style={{ fontSize:12, padding:'6px 10px', background:'white', borderRadius:6, marginBottom:4, display:'flex', alignItems:'center', gap:8 }}>
              ...
              {c.message}
              <span style={{ marginLeft:'auto', color:T.muted, fontSize:11 }}>{c.entries.map(e=>e.class_name).join(', ')}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))', gap:12, marginBottom:20 }}>
        {[
          {label:'Total Entries',   val:entries.length,       icon:'📋', color:T.navy},
          {label:'Batches',         val:uniqueClasses.length, icon:'🏫', color:'#16a34a'},
          {label:'Teachers',        val:uniqueTeachers.length,icon:'👨‍🏫',color:T.purple},
          {label:"Today's Classes", val:todayEntries.length,  icon:'📅', color:T.goldDim},
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'center' }}>
            <div style={{ position:'relative' }}>
              ...
              <input ref={searchRef} placeholder="Search…" value={search} onChange={e=>setSearch(e.target.value)}
                style={{ ...S.inp, padding:'8px 12px 8px 32px', width:200 }}
            ...
            {viewMode!=='admin'&&<button onClick={()=>{ setBulkMode(!bulkMode); setSelectedIds(new Set()) }}
            ...
            return (
              <div key={day} style={{ marginBottom:24 }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                  ...
                  <span style={{ fontSize:18, fontWeight:800, color:isToday?T.goldDim:T.navy }}>{day}</span>
                  {isToday&&<span style={{ background:T.gold, color:T.navy, padding:'2px 10px', borderRadius:999, fontSize:11, fontWeight:800 }}>TODAY</span>}
                  <span style={{ fontSize:12, color:T.muted, background:T.surface, padding:'3px 10px', borderRadius:999 }}>{dayEntries.length} classes</span>
                </div>
                <div style={{ background:'white', borderRadius:12, overflow:'hidden', border:`1px solid ${isToday?T.gold+'44':T.border}`, boxShadow:isToday?`0 4px 20px rgba(240,165,0,0.12)`:'0 2px 8px rgba(0,0,0,.05)' }}>
                  <div style={{ overflowX:'auto' }}>
                    <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                      <thead>
                        <tr style={{ background:isToday?T.navy:T.navyMid }}>
                          <th style={{ padding:'11px 16px', color:isToday?T.gold:'rgba(255,255,255,0.8)', fontWeight:700, fontSize:11, textTransform:'uppercase', letterSpacing:'.08em', minWidth:110, textAlign:'left', borderRight:`1px solid rgba(255,255,255,0.08)` }}>Time</th>
                          {allBatches.map(b=>{ const bs=getBatchStyle(b); return (
                            <th key={b} style={{ padding:'11px 12px', minWidth:130, textAlign:'center' }}>
                              <span style={{ background:bs.bg, color:bs.text, padding:'4px 12px', borderRadius:999, border:`1px solid ${bs.border}`, fontSize:11, fontWeight:700, display:'inline-block' }}>{b}</span>
                            </th>
                          )})}
                        </tr>
                      </thead>
                      <tbody>
                        {periods.length===0&&<tr><td colSpan={allBatches.length+1} style={{ padding:32, textAlign:'center', color:T.muted }}>No entries for {day}</td></tr>}
                        {periods.map((period,pi)=>{
                          ...
                          if (breakEntry) return (
                            <tr key={period} style={{ background:'#fffbeb', borderBottom:`1px solid ${T.border}` }}>
                              <td style={{ padding:'12px 16px', fontWeight:700, color:T.goldDim, fontSize:12, borderRight:`1px solid ${T.border}` }}>{period}</td>
                              <td colSpan={allBatches.length} style={{ padding:'12px', textAlign:'center' }}>
                                <span style={{ fontSize:13, fontWeight:800, color:T.goldDim, letterSpacing:'.12em', textTransform:'uppercase' }}>☕ {breakEntry.subject_name}</span>
                              </td>
                            </tr>
                          )
                          return (
                            <tr key={period} style={{ background:pi%2===0?'white':T.surface, borderBottom:`1px solid ${T.border}` }}>
                              <td style={{ padding:'10px 16px', fontWeight:700, color:T.ink, fontSize:12, borderRight:`1px solid ${T.border}`, whiteSpace:'nowrap' }}>{period}</td>
                              {allBatches.map(batch=>{
                                const cell=pEntries.filter(e=>{ const base=(e.class_name||'').split(' ')[0]; return base===batch||e.class_name===batch })
                                const bs=getBatchStyle(batch)
                                const hasConflict=cell.some(e=>conflicts.some(c=>c.entries.some(en=>en.id===e.id)))
                                return (
                                  <td key={batch} style={{ padding:'8px 10px', verticalAlign:'top', borderRight:`1px solid ${T.border}` }}>
                                    {cell.length?cell.map(e=>(
                                      <div key={e.id} style={{ background:hasConflict?T.redLt:bs.bg, border:hasConflict?`2px solid ${T.red}`:`1px solid ${bs.border}`, borderRadius:8, padding:'8px 10px', cursor:'pointer' }}
                                        onClick={()=>setEditingEntry(e)}>
                                        <div style={{ fontWeight:700, color:hasConflict?T.red:bs.text, fontSize:13, marginBottom:2 }}>{e.subject_name}</div>
                                        ...
                                        {e.room_name&&<div style={{ color:T.muted, fontSize:11 }}>📍 {e.room_name}</div>}
                                      </div>
            ...
            return (
              <div key={day} style={{ marginBottom:24 }}>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
                  <div style={{ width:3, height:24, background:isToday?T.gold:T.navy, borderRadius:2 }}/>
                  <span style={{ fontSize:16, fontWeight:800, color:isToday?T.goldDim:T.navy }}>{day}</span>
                  {isToday&&<span style={{ background:T.gold, color:T.navy, padding:'2px 8px', borderRadius:999, fontSize:10, fontWeight:800 }}>TODAY</span>}
                </div>
                <div style={{ background:'white', borderRadius:12, overflow:'hidden', border:`1px solid ${T.border}`, boxShadow:'0 2px 8px rgba(0,0,0,.05)' }}>
                  <div style={{ overflowX:'auto' }}>
                    <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                      <thead>
                        <tr style={{ background:T.navy }}>
                          <th style={{ padding:'10px 14px', color:'white', fontWeight:700, fontSize:11, textTransform:'uppercase', minWidth:120, textAlign:'left' }}>Period</th>
                          {batches.map(b=>{ const bs=getBatchStyle(b); return <th key={b} style={{ padding:'10px 12px', minWidth:130, textAlign:'center' }}><span style={{ background:bs.bg, color:bs.text, padding:'3px 10px', borderRadius:999, border:`1px solid ${bs.border}`, fontSize:11, fontWeight:700 }}>{b}</span></th> })}
                        </tr>
                      </thead>
                      <tbody>
                        {periods.map((period,pi)=>(
                          <tr key={period} style={{ background:pi%2===0?'white':T.surface, borderBottom:`1px solid ${T.border}` }}>
                            <td style={{ padding:'10px 14px', fontWeight:700, color:T.ink, fontSize:12, whiteSpace:'nowrap', borderRight:`1px solid ${T.border}` }}>{period}</td>
                            {batches.map(batch=>{ const cell=dayEntries.filter(e=>e.class_name===batch&&e.period_name===period); const bs=getBatchStyle(batch); return (
                              <td key={batch} style={{ padding:'8px 10px', verticalAlign:'top', borderRight:`1px solid ${T.border}` }}>
                                {cell.length?cell.map(e=>(
                                  <div key={e.id} style={{ background:bs.bg, border:`1px solid ${bs.border}`, borderRadius:6, padding:'6px 9px', marginBottom:4 }}>
                                    <div style={{ fontWeight:700, color:bs.text }}>{e.subject_name}</div>
                                    {e.teacher_name&&<div style={{ color:T.muted, fontSize:11 }}>👤 {e.teacher_name}</div>}
                                  </div>
                                )):<span style={{ color:'#cbd5e1', fontSize:11 }}>—</span>}
                              </td>
