...
function SecuritySection({ currentUser }) {
  ...
  const changePassword = () => {
    ...
    setPwMsg({ type: "success", text: "Use Admin Panel → Change Password to update the admin password securely." });
      ...
      <div>
        <Card>
          ...
          <div style={{ padding: "12px 16px", borderRadius: 10, background: "#FFF7ED", border: "1px solid #FED7AA", fontSize: 13, color: "#92400E", marginBottom: 16 }}>
            ⚠️ To change the admin password securely, go to <strong>Admin Panel → Change Password</strong>.
          </div>
          ...
          <button onClick={changePassword} style={{ padding: "9px 22px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "none", background: "#1D4ED8", color: "white" }}>
            Update Password
          </button>
        </Card>
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const rows = lines.slice(1).map(line => {
          ...
          return {
            admission_no:  obj["admission_no."] || obj["admission_no"] || null,
            gcc_no:        obj["gcc_no."]        || obj["gcc_no"]        || null,
            name:          obj["name_of_students"] || null,
        ...
        for (let i = 0; i < rows.length; i += 50) {
          const { error } = await supabase.from("students").insert(rows.slice(i, i + 50));
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
      <div>
        ...
        <Card>
          ...
          <p style={{ fontSize: 13, color: "#6B7280", marginTop: 0, marginBottom: 16 }}>Upload a CSV file to import student records.</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            ...
            <div key={table} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", border: "1px solid #E5E7EB", borderRadius: 10, background: "#F9FAFB" }}>
              ...
              <button onClick={() => clearTable(table)} disabled={clearing === table} style={{ padding: "6px 14px", borderRadius: 8, fontSize: 12, cursor: "pointer", border: "1px solid #FECACA", background: "#FEF2F2", color: "#991B1B", fontWeight: 600 }}>
