...
const YT_EMBED = '' // paste YouTube embed ID here e.g. 'dQw4w9WgXcQ'
...
function FAQ() {
  ...
  const items = [
    ...
    ['When does the new batch start?','New batches start in June (summer) and January. Call +91 89742 98074 for the current schedule and availability.'],
    ...
    <div>
      {items.map(([q, a], i) => (
        <div key={i} style={{ borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
          <button onClick={() => setOpen(open === i ? null : i)} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', width:'100%', padding:'1.1rem 0', background:'none', border:'none', color:'#fff', fontSize:'0.95rem', fontWeight:500, cursor:'pointer', gap:'1rem', textAlign:'left' }}>
            {q}
...
export default function LandingPage({ onLogin }) {
  ...
  const [enquiry, setEnquiry]   = useState({ name:'', phone:'', class_interest:'', message:'' })
  const [sending, setSending]   = useState(false)
  const [sent, setSent]         = useState(false)
  const [viewCount]             = useState(() => Math.floor(Math.random() * 400) + 800)
  ...
  // ── Scroll ─────────────────────────────────────────────────────────────────
  useEffect(() => {
    const h = () => { setScrolled(window.scrollY>40); setShowTop(window.scrollY>500) }
  ...
  const handleEnquiry = async e => {
    e.preventDefault()
    if (!enquiry.name || !enquiry.phone) return
    setSending(true)
    try {
      await supabase.from('enquiries').insert([{ ...enquiry, source: 'Landing Page', status: 'New', created_at: new Date().toISOString() }])
      setSent(true)
      setEnquiry({ name:'', phone:'', class_interest:'', message:'' })
  ...
  const card = (extra={}) => ({ background: NAVY2, border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:'1.4rem', transition:'all 0.25s', ...extra })
  ...
  const btnP = { display:'inline-flex', alignItems:'center', gap:8, background:GOLD, color:NAVY, padding:'0.9rem 2rem', borderRadius:50, fontWeight:700, fontSize:'1rem', border:'none', cursor:'pointer', transition:'all 0.2s' }
  const btnS = { display:'inline-flex', alignItems:'center', gap:8, background:'transparent', color:'#fff', padding:'0.9rem 2rem', borderRadius:50, border:'1px solid rgba(255,255,255,0.2)', fontWeight:500, fontSize:'1rem', cursor:'pointer', transition:'all 0.2s' }

  const TICKER_ITEMS = [
    '📢 Admissions Open for 2026-27 Batch — Limited Seats',
    '🏆 2025-26 Results: 66 Students Selected in NVS & Sainik School',
    '📅 Summer Intensive Batch Starts June 1, 2026',
    '📝 Monthly Mock Tests Every Sunday — Register Now',
    '🎓 10+ Years of Excellence · 200+ Officers Produced',
    '📞 Contact: +91 89742 98074',
  ]
  ...
  return (
    <div style={{ background:NAVY, color:'#fff', minHeight:'100vh', fontFamily:"'DM Sans','Segoe UI',sans-serif", overflowX:'hidden' }}>
      ...
      <style>{`
        ...
        .gnsi-card:hover { border-color:rgba(201,168,76,0.35)!important; transform:translateY(-4px)!important }
        .gnsi-event:hover { border-color:rgba(201,168,76,0.25)!important; transform:translateX(4px)!important }
        .gnsi-facility:hover { border-color:rgba(201,168,76,0.3)!important; transform:translateY(-3px)!important }
        .gnsi-result:hover { border-color:rgba(201,168,76,0.3)!important; transform:translateY(-4px)!important }
      ...
      {/* ── TICKER ── */}
      <div style={{ background:`linear-gradient(90deg,${GOLD},#b8922a)`, padding:'8px 0', overflow:'hidden', position:'relative', zIndex:101 }}
        onMouseEnter={()=>setTP(true)} onMouseLeave={()=>setTP(false)}>
        <div style={{ display:'flex', gap:'4rem', whiteSpace:'nowrap', animation:`ticker ${tickerPaused?'none':'28s'} linear infinite` }}>
          {[...TICKER_ITEMS,...TICKER_ITEMS].map((t,i) => (
            <span key={i} style={{ fontSize:'0.8rem', fontWeight:600, color:NAVY, flexShrink:0 }}>{t}</span>
          ))}
        </div>
      </div>

      {/* ── NAV ── */}
      <nav style={{ position:'fixed', top:32, left:0, right:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.9rem 5%', background: scrolled?'rgba(11,30,61,0.96)':'rgba(11,30,61,0.85)', backdropFilter:'blur(20px)', borderBottom:'1px solid rgba(201,168,76,0.15)', transition:'all 0.3s', boxShadow: scrolled?'0 4px 24px rgba(0,0,0,0.3)':'none' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <span style={{ fontSize:'0.6rem', color:MUTED, fontWeight:400, letterSpacing:'0.1em', textTransform:'uppercase', display:'block' }}>Khangabok, Manipur</span>
          </div>
        </div>
        <ul className="gnsi-nav-links" style={{ display:'flex', alignItems:'center', gap:'1.5rem', listStyle:'none', margin:0, padding:0 }}>
          {[['notices','Notices'],['results','Results'],['gallery','Gallery'],['events','Events'],['enquiry','Enquire'],['contact','Contact']].map(([id,label]) => (
            <li key={id}><button onClick={()=>scrollTo(id)} style={{ background:'none', border:'none', color:'rgba(255,255,255,0.75)', fontSize:'0.85rem', cursor:'pointer', fontFamily:'inherit', transition:'color 0.2s' }} onMouseEnter={e=>e.target.style.color=GOLD_L} onMouseLeave={e=>e.target.style.color='rgba(255,255,255,0.75)'}>{label}</button></li>
          ))}
          <li><button onClick={onLogin} style={{ background:GOLD, color:NAVY, padding:'0.5rem 1.4rem', borderRadius:50, fontWeight:700, fontSize:'0.85rem', border:'none', cursor:'pointer', transition:'all 0.2s' }} onMouseEnter={e=>e.target.style.background=GOLD_L} onMouseLeave={e=>e.target.style.background=GOLD}>Login →</button></li>
        </ul>
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div style={{ position:'fixed', top:72, left:0, right:0, background:'rgba(11,30,61,0.98)', zIndex:99, padding:'1rem 5%', borderBottom:'1px solid rgba(201,168,76,0.15)' }}>
          {[['notices','📢 Notices'],['results','🏆 Results'],['gallery','📸 Gallery'],['events','📅 Events'],['enquiry','✏️ Enquire'],['contact','📍 Contact']].map(([id,label]) => (
            <button key={id} onClick={()=>scrollTo(id)} style={{ display:'block', width:'100%', textAlign:'left', background:'none', border:'none', color:'rgba(255,255,255,0.8)', padding:'12px 0', fontSize:15, cursor:'pointer', borderBottom:'1px solid rgba(255,255,255,0.06)', fontFamily:'inherit' }}>{label}</button>
          ))}
          <button onClick={onLogin} style={{ background:GOLD, color:NAVY, padding:'0.8rem 1.4rem', borderRadius:50, fontWeight:700, fontSize:'0.9rem', border:'none', cursor:'pointer', marginTop:12, width:'100%' }}>Login to Portal →</button>
        </div>
      )}

      {/* ── HERO ── */}
      <section style={{ minHeight:'100vh', display:'flex', alignItems:'center', padding:'9rem 5% 4rem', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:2, maxWidth:660, animation:'fadeUp 0.8s ease both' }}>
          {/* View counter */}
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.6)', fontSize:'0.72rem', padding:'0.35rem 0.9rem', borderRadius:50, marginBottom:'1rem' }}>
          ...
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(201,168,76,0.12)', border:'1px solid rgba(201,168,76,0.3)', color:GOLD_L, fontSize:'0.75rem', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', padding:'0.4rem 1rem', borderRadius:50, marginBottom:'1.5rem', display:'block' }}>
            <span style={{ width:6, height:6, background:GOLD, borderRadius:'50%', display:'inline-block', marginRight:8 }} />
          ...
          <p style={{ fontSize:'1.05rem', lineHeight:1.8, color:'rgba(255,255,255,0.65)', maxWidth:520, marginBottom:'2.5rem', fontWeight:300 }}>
            Guidance Navodaya & Sainik Institute — Khangabok's premier coaching centre for Navodaya, Sainik, and RMS entrance examination preparation. Over 200 officers produced in 10 years.
          </p>
          ...
          <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap', marginBottom:'2rem' }}>
            ...
            <button onClick={onLogin} style={btnS} onMouseEnter={e=>e.currentTarget.style.borderColor=GOLD} onMouseLeave={e=>e.currentTarget.style.borderColor='rgba(255,255,255,0.2)'}>Staff Login</button>
            <a href={`https://wa.me/${WA_NUM}?text=Hello, I want to enquire about admissions at GNSI Khangabok`} target="_blank" rel="noreferrer"
              style={{ display:'inline-flex', alignItems:'center', gap:8, background:'#25D366', color:'#fff', padding:'0.9rem 1.5rem', borderRadius:50, fontWeight:600, fontSize:'1rem', textDecoration:'none' }}>
              💬 WhatsApp
            </a>
          </div>
          ...
          <div style={{ display:'flex', gap:'0.8rem', flexWrap:'wrap' }}>
            {['✅ 95% Selection Rate','🏆 10+ Years','🛡️ Safe Hostel','📱 Digital Portal'].map(b => (
              <span key={b} style={{ fontSize:'0.75rem', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:50, padding:'0.3rem 0.8rem', color:'rgba(255,255,255,0.7)' }}>{b}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <div ref={statsRef} style={{ padding:'3rem 5%', borderTop:'1px solid rgba(255,255,255,0.06)', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))', gap:'2rem', textAlign:'center', background:'rgba(255,255,255,0.01)' }}>
        {[
          ...
          { num:95, suffix:'%', label:'Selection rate' },
          { num:200, suffix:'+', label:'Officers produced' },
          { num:15, suffix:'+', label:'Portal modules' },
        ].map(s => (
          <div key={s.label}>
            <div style={{ fontFamily:'Georgia,serif', fontSize:'2.5rem', fontWeight:700, color:GOLD, lineHeight:1 }}>
              <Counter value={s.num} suffix={s.suffix} />
            </div>
            <div style={{ fontSize:'0.82rem', color:MUTED, marginTop:6 }}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* ── SELECTION PROGRESS BARS ── */}
      <section style={{ padding:'5rem 5%', background:'rgba(255,255,255,0.015)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:mobile?'1fr':'1fr 1fr', gap:'3rem', marginTop:'2rem' }}>
          <div>
            {[
              { label:'NVS (Navodaya) Selection Rate', pct:94 },
              { label:'Sainik School Selection Rate', pct:88 },
              { label:'Student Satisfaction', pct:98 },
              { label:'Hostel Occupancy', pct:92 },
              { label:'Faculty Expertise Rating', pct:96 },
            ].map(item => (
              <div key={item.label} style={{ marginBottom:'1.4rem' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.85rem', marginBottom:6 }}>
                  <span style={{ color:'rgba(255,255,255,0.8)', fontWeight:500 }}>{item.label}</span>
                  <span style={{ color:GOLD, fontWeight:700 }}>{item.pct}%</span>
                </div>
                <div style={{ height:8, background:'rgba(255,255,255,0.08)', borderRadius:99, overflow:'hidden' }}>
                  <div style={{ height:'100%', borderRadius:99, background:`linear-gradient(90deg,${GOLD},${GOLD_L})`, width:statsInView?`${item.pct}%`:'0%', transition:'width 1.5s cubic-bezier(0.4,0,0.2,1)' }} />
                </div>
              </div>
            ))}
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', alignContent:'start' }}>
            {[
              { icon:'🏫', label:'Est. 2016', desc:'Over a decade of trust' },
              { icon:'👨‍🎓', label:'500+ Alumni', desc:'Across Manipur' },
              { icon:'🏆', label:'#1 Rated', desc:'In Thoubal District' },
              { icon:'📱', label:'Digital ERP', desc:'Custom-built portal' },
            ].map(f => (
              <div key={f.label} style={{ ...card(), textAlign:'center', padding:'1.5rem 1rem' }} className="gnsi-facility">
                <div style={{ fontSize:'1.8rem', marginBottom:'0.6rem' }}>{f.icon}</div>
                <div style={{ fontSize:'0.9rem', fontWeight:700, marginBottom:4 }}>{f.label}</div>
                <div style={{ fontSize:'0.75rem', color:MUTED }}>{f.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="notices" style={{ padding:'5.5rem 5%' }}>
        ...
        <p style={sS}>Stay updated with the latest news, admissions, and exam schedules.</p>
        {lN ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'1rem' }}>{[1,2,3,4].map(i=><Sk key={i} h={160}/>)}</div>
        ) : notices.length === 0 ? (
          <div style={{ textAlign:'center', color:MUTED, padding:'3rem', ...card() }}>
            <div style={{ fontSize:'2rem', marginBottom:12 }}>📢</div>No public notices at the moment.
          </div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'1rem' }}>
            {notices.map(n => (
              <div key={n.id} onClick={()=>setPreview(n)} className="gnsi-card"
                style={{ ...card(), borderLeft:`4px solid ${n.priority==='Urgent'?'#dc2626':n.priority==='Important'?'#d97706':'rgba(201,168,76,0.3)'}`, cursor:'pointer', position:'relative' }}>
                {n.pinned && <span style={{ position:'absolute', top:10, right:10, fontSize:14 }}>📌</span>}
                <NoticeTag priority={n.priority} category={n.category} />
      ...
      <section id="results" style={{ padding:'5.5rem 5%', background:'rgba(255,255,255,0.015)' }}>
        ...
        <p style={sS}>Consistently clearing Navodaya, Sainik, and RMS entrance examinations.</p>
        {lR ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:'1.2rem' }}>{[1,2,3,4].map(i=><Sk key={i} h={140}/>)}</div>
        ) : results.length === 0 ? (
          <div style={{ textAlign:'center', color:MUTED, padding:'3rem', ...card() }}><div style={{ fontSize:'2rem', marginBottom:12 }}>🏆</div>Results coming soon.</div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:'1.2rem' }}>
            {results.map(r => (
              <div key={r.id} className="gnsi-result" style={{ ...card(), textAlign:'center', position:'relative', overflow:'hidden' }}>
      ...
      <section id="gallery" style={{ padding:'5.5rem 5%' }}>
        ...
        {lG ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'1rem' }}>{[1,2,3].map(i=><Sk key={i} h={180}/>)}</div>
        ) : gallery.length === 0 ? (
          <div style={{ display:'grid', gridTemplateColumns:mobile?'repeat(2,1fr)':'repeat(4,1fr)', gap:'1rem' }}>
            {[{span:mobile?2:2,h:mobile?180:300,icon:'🏫',label:'GNSI Campus'},{span:1,h:140,icon:'📚',label:'Study Hall'},{span:1,h:140,icon:'🏆',label:'Prize Distribution'},{span:1,h:140,icon:'👨‍🎓',label:'Selected Students'},{span:1,h:140,icon:'⚽',label:'Sports Day'}].map((item,i) => (
              <div key={i} style={{ gridColumn:`span ${item.span}`, ...card({ padding:0, minHeight:item.h, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8 }) }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:'1rem' }}>
            {gallery.map((post,i) => (
              <div key={post.id} style={{ ...card({ padding:0, overflow:'hidden', position:'relative', minHeight:180, gridColumn:i===0?'span 2':'span 1', cursor:'pointer' }) }}
                onMouseEnter={e=>e.currentTarget.style.transform='scale(1.02)'}
                onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>
                {post.image_url ? <img src={post.image_url} alt={post.caption||post.title} style={{ width:'100%', height:'100%', minHeight:180, objectFit:'cover', display:'block' }} />
                  : <div style={{ width:'100%', minHeight:180, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8, background:NAVY2 }}>
                      <span style={{ fontSize:'2rem' }}>📸</span>
                      <span style={{ fontSize:'0.78rem', color:MUTED }}>{post.content_type}</span>
                    </div>}
                <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top,rgba(11,30,61,0.8) 0%,transparent 50%)', display:'flex', alignItems:'flex-end', padding:'1rem' }}>
                  <div>
                    ...
                    <div style={{ fontSize:'0.7rem', color:MUTED }}>{post.platform} · {post.post_date}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <p style={{ textAlign:'center', color:MUTED, fontSize:'0.8rem', marginTop:'1.5rem' }}>📸 Photos managed from the Social module · Mark posts as public to show here</p>
      </section>
      ...
      <section id="events" style={{ padding:'5.5rem 5%', background:'rgba(255,255,255,0.015)' }}>
        ...
        {lE ? (
          <div style={{ display:'flex', flexDirection:'column', gap:'1rem', marginTop:'2rem' }}>{[1,2,3].map(i=><Sk key={i} h={90}/>)}</div>
        ) : events.length === 0 ? (
          <div style={{ textAlign:'center', color:MUTED, padding:'3rem', ...card({ marginTop:'2rem' }) }}><div style={{ fontSize:'2rem', marginBottom:12 }}>📅</div>No upcoming events. Check back soon.</div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:'1rem', marginTop:'2rem' }}>
            {events.map(ev => {
              ...
              return (
                <div key={ev.id} className="gnsi-event" style={{ ...card(), display:'flex', gap:'1.5rem', alignItems:'flex-start' }}>
      ...
      <section style={{ padding:'5.5rem 5%' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))', gap:'1.2rem', marginTop:'2.5rem' }}>
          {[
            { i:'RD', name:'Romen Devi', sub:'Parent · Thoubal', text:'"My son got selected in Jawahar Navodaya Vidyalaya Class 6 after just one year at GNSI. The teachers are dedicated and the results speak for themselves."' },
            { i:'KS', name:'Kiran Singh', sub:'Parent · Imphal', text:'"The hostel facility is clean and safe. GNSI gave my daughter the discipline and knowledge to crack Sainik School entrance on her first attempt."' },
            { i:'BM', name:'Bikash Meetei', sub:'NVS Selected 2025', text:'"I joined GNSI in Class 5 and got selected in NVS. The weekly mock tests helped me identify weak areas and improve quickly."' },
          ].map(t => (
            <div key={t.name} style={card()} className="gnsi-card">
      ...
      <section style={{ padding:'5.5rem 5%', background:'rgba(255,255,255,0.015)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:'1.2rem', marginTop:'2.5rem' }}>
          {[['🏠','Boarding Hostel','Safe, clean and 24/7 supervised hostel for outstation students.'],['📚','Study Hall','Dedicated quiet study area with proper lighting for focused prep.'],['🍽️','Mess & Nutrition','Balanced nutritious meals served three times daily.'],['⚽','Sports Ground','Outdoor play area essential for Sainik School physical fitness.'],['👨‍🏫','Expert Faculty','Experienced teachers specializing in NVS and Sainik syllabus.'],['📱','Digital Portal','Parents track fees, attendance and results online anytime.']].map(([icon,name,desc]) => (
            <div key={name} style={{ ...card(), textAlign:'center' }} className="gnsi-facility">
      ...
      {YT_EMBED && (
        <section style={{ padding:'5.5rem 5%' }}>
          ...
          <div style={{ borderRadius:20, overflow:'hidden', border:'1px solid rgba(201,168,76,0.2)', marginTop:'2rem', aspectRatio:'16/9', maxWidth:800, margin:'2rem auto 0' }}>
            <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${YT_EMBED}`} title="GNSI Institute" frameBorder="0" allowFullScreen style={{ display:'block' }} />
          </div>
        </section>
      )}
      ...
      <section id="enquiry" style={{ padding:'5.5rem 5%', background:'rgba(255,255,255,0.015)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:mobile?'1fr':'1fr 1fr', gap:'3rem', alignItems:'start', maxWidth:900 }}>
          <div style={{ ...card(), border:'1px solid rgba(201,168,76,0.2)', position:'relative', overflow:'hidden' }}>
            ...
            {sent ? (
              <div style={{ textAlign:'center', padding:'2rem 0' }}>
                ...
                <p style={{ color:'rgba(255,255,255,0.6)', fontSize:'0.9rem', lineHeight:1.6 }}>Our team will call you back within 24 hours. You can also WhatsApp us directly.</p>
              ...
              <form onSubmit={handleEnquiry}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                  <div>
                    <label style={{ display:'block', fontSize:'0.75rem', fontWeight:600, color:MUTED, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.06em' }}>Student / Parent Name *</label>
                    <input style={inp} placeholder="Full name" required value={enquiry.name} onChange={e=>setEnquiry({...enquiry,name:e.target.value})} />
                  </div>
                  <div>
                    <label style={{ display:'block', fontSize:'0.75rem', fontWeight:600, color:MUTED, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.06em' }}>Phone Number *</label>
                    <input style={inp} placeholder="+91 XXXXX XXXXX" required value={enquiry.phone} onChange={e=>setEnquiry({...enquiry,phone:e.target.value})} />
                  </div>
                  <div>
                    <label style={{ display:'block', fontSize:'0.75rem', fontWeight:600, color:MUTED, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.06em' }}>Class / Course Interest</label>
                    <select style={{ ...inp, cursor:'pointer' }} value={enquiry.class_interest} onChange={e=>setEnquiry({...enquiry,class_interest:e.target.value})}>
                      <option value="">Select class…</option>
                      {['NVS Class 6','NVS Class 9','Sainik School Class 6','Sainik School Class 9','RMS','Other'].map(c=><option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ display:'block', fontSize:'0.75rem', fontWeight:600, color:MUTED, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.06em' }}>Message (optional)</label>
                    <textarea rows={3} style={{ ...inp, resize:'vertical' }} placeholder="Any specific questions?" value={enquiry.message} onChange={e=>setEnquiry({...enquiry,message:e.target.value})} />
                  </div>
                  <button type="submit" disabled={sending} style={{ ...btnP, width:'100%', justifyContent:'center', opacity:sending?0.7:1 }}>
                    {sending ? '⏳ Sending…' : '📨 Send Enquiry'}
                  </button>
                  <p style={{ fontSize:'0.72rem', color:MUTED, textAlign:'center' }}>Or WhatsApp us directly at <a href={`https://wa.me/${WA_NUM}`} target="_blank" rel="noreferrer" style={{ color:GOLD_L }}>+91 89742 98074</a></p>
                </div>
              </form>
            )}
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
            {[['📍','Address','Khangabok Sorok Wangma, Near Community Hall, Thoubal, Manipur — 795138'],['📞','Phone','+91 89742 98074'],['🕐','Office Hours','Mon–Sat: 8:00 AM – 6:00 PM'],['🌐','Portal','gnsi-erp.vercel.app']].map(([icon,label,val]) => (
              <div key={label} style={{ ...card(), display:'flex', gap:'1rem', alignItems:'flex-start' }} className="gnsi-card">
                <div style={{ fontSize:'1.3rem', minWidth:36 }}>{icon}</div>
                <div>
                  <div style={{ fontSize:'0.72rem', color:MUTED, marginBottom:3, textTransform:'uppercase', letterSpacing:'0.06em' }}>{label}</div>
      ...
      <section id="contact" style={{ padding:'5.5rem 5%' }}>
        ...
        <div style={{ borderRadius:20, overflow:'hidden', border:'1px solid rgba(201,168,76,0.2)', marginTop:'2rem', height:360 }}>
          <iframe src={MAPS_SRC} width="100%" height="100%" frameBorder="0" style={{ display:'block' }} allowFullScreen title="GNSI Location" />
        </div>
      </section>
      ...
      <div style={{ margin:'0 5% 5.5rem', background:`linear-gradient(135deg,#1e3d70 0%,${NAVY2} 100%)`, border:'1px solid rgba(201,168,76,0.25)', borderRadius:28, padding:'5rem 3rem', textAlign:'center', position:'relative', overflow:'hidden' }}>
        ...
        <p style={{ color:'rgba(255,255,255,0.5)', fontSize:'0.95rem', maxWidth:420, margin:'0 auto 2.5rem', lineHeight:1.7 }}>Limited seats. Don't miss this opportunity to prepare your child for a bright future as an officer.</p>
      ...
      <footer style={{ padding:'3rem 5% 2rem', borderTop:'1px solid rgba(255,255,255,0.06)', background:NAVY2 }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:'2rem', marginBottom:'2rem' }}>
          <div>
            ...
            <a href={`https://wa.me/${WA_NUM}`} target="_blank" rel="noreferrer" style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(37,211,102,0.1)', border:'1px solid rgba(37,211,102,0.2)', color:'#4ade80', padding:'6px 12px', borderRadius:8, textDecoration:'none', fontSize:'0.8rem', fontWeight:600, marginTop:'1rem' }}>💬 WhatsApp Us</a>
          </div>
          <div>
          ...
          <div>
            <div style={{ fontSize:'0.72rem', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:MUTED, marginBottom:'1rem' }}>Portal Access</div>
            <button onClick={onLogin} style={{ display:'block', background:GOLD, color:NAVY, padding:'8px 16px', borderRadius:8, border:'none', cursor:'pointer', fontWeight:700, fontSize:'0.85rem', marginBottom:8, fontFamily:'inherit' }}>Staff Login →</button>
            <p style={{ fontSize:'0.75rem', color:'rgba(255,255,255,0.3)', lineHeight:1.5 }}>Authorized staff only. Contact admin for access.</p>
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'1rem', paddingTop:'2rem', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ fontSize:'0.75rem', color:MUTED }}>© {new Date().getFullYear()} Guidance Navodaya & Sainik Institute, Khangabok, Manipur. All rights reserved.</div>
          <div style={{ fontSize:'0.75rem', color:MUTED }}>Built with GNSI Portal ERP</div>
        </div>
      </footer>

      {/* ── WHATSAPP FLOATING BUTTON ── */}
      <a href={`https://wa.me/${WA_NUM}?text=Hello, I want to enquire about admissions at GNSI Khangabok`} target="_blank" rel="noreferrer"
        style={{ position:'fixed', bottom:mobile?80:24, right:24, zIndex:999, width:56, height:56, borderRadius:'50%', background:'#25D366', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.6rem', boxShadow:'0 4px 20px rgba(37,211,102,0.5)', textDecoration:'none', animation:'bounce 2s infinite' }}
      ...
      {showTop && (
        <button onClick={()=>window.scrollTo({top:0,behavior:'smooth'})}
          style={{ position:'fixed', bottom:mobile?140:90, right:24, zIndex:998, width:44, height:44, borderRadius:'50%', background:'rgba(201,168,76,0.2)', border:`1px solid rgba(201,168,76,0.4)`, color:GOLD_L, fontSize:'1.1rem', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(8px)' }}>
          ↑
        </button>
      )}

      {preview && <NoticeModal notice={preview} onClose={()=>setPreview(null)} />}
    </div>
  )
}