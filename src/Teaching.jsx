...
const COURSES = ['Sainik', 'Navodaya', 'Foundation', 'Combined']

const BATCHES = {
  Sainik:     ['Achiever', 'Leader', 'Champion'],
  Navodaya:   ['Lakshya', 'Ummed'],
  Foundation: ['Elite', 'Prime'],
  Combined:   ['Achiever', 'Leader', 'Champion'],
}
...
const MISSED_REASONS = ['Absent', 'Holiday', 'Exam', 'Other']
...
const emptyLog = {
  course: '', batch: '', subject_name: '', teacher_name: '',
...
function LogForm({ form, setForm, onSubmit, saving, timetable, staff, onCancel, editMode = false }) {
  const batches = form.course ? BATCHES[form.course] || [] : []

  const handlePeriodSelect = (e) => {
    const pn = parseInt(e.target.value)
    if (!pn || !form.course || !form.batch) { setForm(f => ({ ...f, period_number: pn || '' })); return }
    const dayName = DAYS[new Date().getDay() - 1] || 'Monday'
    const slot = timetable.find(t =>
      t.course === form.course && t.batch === form.batch &&
      t.period_number === pn && t.day_of_week === dayName
    )
    if (slot) {
      setForm(f => ({ ...f, period_number: pn, subject_name: slot.subject_name, teacher_name: slot.teacher_name || f.teacher_name }))
    } else {
    ...
    <form onSubmit={onSubmit}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        <div>
          ...
          <select value={form.course} onChange={e => setForm(f => ({ ...f, course: e.target.value, batch: '' }))} required style={S.select}>
        ...
        <div>
          ...
          <select value={form.batch} onChange={e => setForm(f => ({ ...f, batch: e.target.value }))} required style={S.select}>
...
function TabLogs({ logs, loading, fetchLogs, timetable, staff }) {
  ...
  const checkDuplicate = useCallback((f) => {
    if (!f.course || !f.batch || !f.subject_name || !f.teaching_date) return false
    return logs.some(l =>
      l.course === f.course && l.batch === f.batch &&
  ...
  const handleAdd = async (e) => {
    ...
    if (checkDuplicate(form)) {
      setDupWarn(`⚠️ A log for ${form.course} / ${form.batch} / ${form.subject_name} on ${form.teaching_date} already exists.`)
      return
    }
    setDupWarn('')
    setSaving(true)
    const { error } = await supabase.from('teaching_logs').insert([{
      course: form.course, batch: form.batch, subject_name: form.subject_name,
      teacher_name: form.teacher_name, teaching_date: form.teaching_date,
      topic_taught: form.topic_taught, classwork: form.classwork,
      homework: form.homework, remarks: form.remarks,
  ...
  const handleEdit = async (e) => {
    ...
    if (checkDuplicate(editForm)) {
      alert(`⚠️ Duplicate log exists for ${editForm.course} / ${editForm.batch} / ${editForm.subject_name} on ${editForm.teaching_date}`)
      return
    }
    setEditSaving(true)
    const { error } = await supabase.from('teaching_logs').update({
      course: editForm.course, batch: editForm.batch, subject_name: editForm.subject_name,
      teacher_name: editForm.teacher_name, teaching_date: editForm.teaching_date,
      topic_taught: editForm.topic_taught, classwork: editForm.classwork,
      homework: editForm.homework, remarks: editForm.remarks,
      period_number: editForm.period_number || null,
    }).eq('id', editId)
    if (error) alert('Error: ' + error.message)
  ...
  const startEdit = (item) => {
    setEditId(item.id)
    setEditForm({
      course: item.course || '', batch: item.batch || '',
      subject_name: item.subject_name || '', teacher_name: item.teacher_name || '',
      teaching_date: item.teaching_date || today(), topic_taught: item.topic_taught || '',
      classwork: item.classwork || '', homework: item.homework || '',
      remarks: item.remarks || '', period_number: item.period_number || '',
    })
  }
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter(item => {
      const matchSearch = ['course','batch','subject_name','teacher_name','topic_taught','classwork','homework','remarks']
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          ...
          { label: 'Subjects', value: uniqueSubjects.length, color: '#7c3aed', bg: '#f3e8ff', icon: '📚' },
      ...
      <div style={S.card}>
        ...
        {showForm && <LogForm form={form} setForm={setForm} onSubmit={handleAdd} saving={saving} timetable={timetable} staff={staff} />}
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                {['#','Date','Course','Batch','Subject','Teacher','Topic','Homework','Actions'].map(h => (
            ...
            <tbody>
              {filtered.map((item, i) => (
                <>
                  <tr key={item.id} style={{ borderBottom: editId === item.id ? 'none' : '1px solid #f1f5f9', background: editId === item.id ? '#f8f4ff' : 'white' }}>
                    ...
                    <td style={{ padding: '12px 14px', color: '#64748b', fontSize: '13px' }}>{item.batch || '-'}</td>
                  ...
                  {editId === item.id && (
                    <tr key={`edit-${item.id}`} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      <td colSpan={9} style={{ padding: '16px 24px', background: '#f8f4ff' }}>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={9} style={{ padding: '32px', textAlign: 'center', color: '#94a3b8' }}>No teaching logs found</td></tr>
              )}
            </tbody>
          </table>
        </div>
    ...
    <div style={S.card}>
      ...
      {selectedDay && (
        <div style={{ marginTop: '20px', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
          ...
          {selectedLogs.length === 0 && selectedMissed.length === 0 && (
            <p style={{ color: '#94a3b8', fontSize: '14px' }}>No activity recorded.</p>
          )}
          {selectedLogs.map(l => (
            <div key={l.id} style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '12px 16px', marginBottom: '10px' }}>
              <div style={{ fontWeight: '700', color: '#15803d', fontSize: '14px' }}>{l.subject_name} — {l.course} / {l.batch}</div>
          ...
          {selectedMissed.map(m => (
            <div key={m.id} style={{ background: '#fff1f2', border: '1px solid #fecaca', borderRadius: '8px', padding: '12px 16px', marginBottom: '10px' }}>
              <div style={{ fontWeight: '700', color: '#dc2626', fontSize: '14px' }}>❌ Missed — {m.subject_name} ({m.course} / {m.batch})</div>
...
function TabSyllabus({ logs }) {
  const [syllabus, setSyllabus] = useState([])
  const [loading, setLoading]   = useState(true)
  const [form, setForm]         = useState({ course: '', batch: '', subject_name: '', total_topics: '' })
  const [saving, setSaving]     = useState(false)
  const [showForm, setShowForm] = useState(false)
  ...
  const handleSave = async (e) => {
    ...
    }], { onConflict: 'course,batch,subject_name' })
    if (error) alert('Error: ' + error.message)
    else { setForm({ course: '', batch: '', subject_name: '', total_topics: '' }); setShowForm(false); fetchSyllabus() }
    setSaving(false)
  }
  ...
  const getCompleted = (row) =>
    logs.filter(l => l.course === row.course && l.batch === row.batch && l.subject_name === row.subject_name).length
  ...
  return (
    <div style={S.card}>
      ...
      {showForm && (
        <form onSubmit={handleSave} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: '12px', alignItems: 'flex-end', marginBottom: '24px', padding: '16px', background: '#f8fafc', borderRadius: '8px' }}>
          <div>
            <label style={S.label}>Course</label>
            <select value={form.course} onChange={e => setForm(f => ({ ...f, course: e.target.value, batch: '' }))} required style={S.select}>
          ...
          <div>
            ...
            <select value={form.batch} onChange={e => setForm(f => ({ ...f, batch: e.target.value }))} required style={S.select}>
              <option value="">Select</option>
              {(BATCHES[form.course] || []).map(b => <option key={b} value={b}>{b}</option>)}
            </select>
          </div>
          <div>
            <label style={S.label}>Subject</label>
      ...
      {loading ? <div style={{ textAlign: 'center', padding: '32px', color: '#64748b' }}>⏳ Loading...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {syllabus.length === 0 && <div style={{ textAlign: 'center', padding: '32px', color: '#94a3b8' }}>No syllabus defined yet. Add one above.</div>}
          {syllabus.map(row => {
              ...
              <div key={row.id} style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  <div>
                    ...
                    <span style={{ marginLeft: '10px', fontSize: '12px', color: '#64748b' }}>{row.course} / {row.batch}</span>
                  </div>
...
function TabTimetable({ timetable, fetchTimetable, staff }) {
  const [form, setForm]     = useState({ course: '', batch: '', subject_name: '', teacher_name: '', day_of_week: 'Monday', period_number: 1, start_time: '', end_time: '' })
  const [saving, setSaving] = useState(false)
  const [showForm, setShowForm] = useState(false)
  const [viewCourse, setViewCourse] = useState('Sainik')
  const [viewBatch, setViewBatch]   = useState('Achiever')

  const handleSave = async (e) => {
    e.preventDefault()
    setSaving(true)
    const { error } = await supabase.from('teaching_timetable').upsert([{
      course: form.course, batch: form.batch, subject_name: form.subject_name,
      teacher_name: form.teacher_name, day_of_week: form.day_of_week,
      period_number: parseInt(form.period_number),
      start_time: form.start_time || null, end_time: form.end_time || null,
    }], { onConflict: 'course,batch,day_of_week,period_number' })
    if (error) alert('Error: ' + error.message)
    else { setShowForm(false); fetchTimetable() }
    setSaving(false)
  }
  ...
  const getSlot = (day, period) =>
    timetable.find(t => t.course === viewCourse && t.batch === viewBatch && t.day_of_week === day && t.period_number === period)
  ...
  return (
    <div style={S.card}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', alignItems: 'center' }}>
        ...
        <select value={viewCourse} onChange={e => { setViewCourse(e.target.value); setViewBatch(BATCHES[e.target.value]?.[0] || '') }} style={{ ...S.select, width: 'auto' }}>
          {COURSES.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={viewBatch} onChange={e => setViewBatch(e.target.value)} style={{ ...S.select, width: 'auto' }}>
          {(BATCHES[viewCourse] || []).map(b => <option key={b} value={b}>{b}</option>)}
        </select>
      </div>
...
function TabReports({ logs, missed, staff }) {
  const [month, setMonth]   = useState(currentYearMonth())
  const [teacher, setTeacher] = useState('All')

  const teachers = [...new Set(logs.map(l => l.teacher_name).filter(Boolean))]

  const monthLogs   = logs.filter(l => l.teaching_date?.startsWith(month) && (teacher === 'All' || l.teacher_name === teacher))
  ...
  const byTeacher = useMemo(() => {
    ...
    monthLogs.forEach(l => {
      ...
      if (!map[l.teacher_name]) map[l.teacher_name] = { logs: [], subjects: new Set(), dates: new Set() }
    ...
    <div style={S.card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button onClick={handlePrint} style={S.btn('#7c3aed')}>🖨️ Print / PDF</button>
        </div>
      </div>
            ...
            <div style={{ marginTop: '10px', fontSize: '13px', color: '#64748b' }}>
              {[...data.logs].slice(0, 5).map(l => (
                <div key={l.id} style={{ borderBottom: '1px solid #f1f5f9', padding: '4px 0' }}>
                  {fmtDate(l.teaching_date)} — {l.subject_name}: <em>{l.topic_taught}</em>
                </div>
              ))}
      ...
      {Object.keys(byTeacher).length === 0 && (
        <div style={{ textAlign: 'center', padding: '32px', color: '#94a3b8' }}>No logs for this period.</div>
      )}
    </div>
  )
}
        ...
        {results.map(l => (
          <div key={l.id} style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '14px 18px' }}>
            ...
            <div style={{ fontSize: '13px', color: '#64748b' }}>
              {l.course} / {l.batch} | {l.subject_name} | 👨‍🏫 {l.teacher_name || '-'}
            </div>
...
function Teaching() {
  ...
  const fetchTimetable = useCallback(async () => {
    const { data } = await supabase.from('teaching_timetable').select('*').order('period_number')
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '14px', margin: '4px 0 0' }}>Daily logs, syllabus tracking, timetable, reports & topic search</p>
      </div>
      ...
      {activeTab === 'logs'      && <TabLogs      logs={logs} loading={loading} fetchLogs={fetchLogs} timetable={timetable} staff={staff} />}
      {activeTab === 'calendar'  && <TabCalendar  logs={logs} missed={missed} />}
      {activeTab === 'syllabus'  && <TabSyllabus  logs={logs} />}
      {activeTab === 'timetable' && <TabTimetable timetable={timetable} fetchTimetable={fetchTimetable} staff={staff} />}
      {activeTab === 'reports'   && <TabReports   logs={logs} missed={missed} staff={staff} />}
