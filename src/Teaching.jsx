...
const ALL_TABS = [
  ...
  { key:'performance', label:'Class Test Scores', icon:PIcon.report },
...
// names ('Administrator', 'Co-Admin', 'Teaching', 'Teaching + Admin', …).
// Those never matched, so real admins and teachers got no tab bar at all.
// Same class of fix as Leave/Fees/Hostel: map real names onto the keys.
function teachingRoleKey(role) {
  const r = String(role || '').trim().toLowerCase()
  if (isAdminRole(role) || ['admin', 'administrator', 'co-admin', 'teaching + admin'].includes(r)) return 'admin'
  if (r === 'teaching') return 'teacher'
...
function Toast({ msg, color=PX.navy, onDone }) {
  ...
  return (
    <div role="status" aria-live="polite" style={{
      ...
      boxShadow:'0 18px 40px -18px rgba(19,42,79,.45), 0 2px 6px rgba(19,42,79,.08)', maxWidth:'92vw',
      color:PX.ink, display:'flex', alignItems:'center', gap:10,
      animation:'tchToastIn .2s ease',
    }}>
...
function useToast() {
  ...
  const show = useCallback((msg, color=PX.navy) => setToast({ msg, color }), [])
...
function ConfirmModal({ title, message, confirmLabel='Confirm', danger=false, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:99999, background:'rgba(14,32,63,.55)', backdropFilter:'blur(3px)', display:'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent:'center' }} onClick={onCancel}>
      <div role="dialog" aria-modal="true" aria-label={title}
        style={{ background:'#fff', borderRadius: isMobile ? '20px 20px 0 0' : 18, padding:24, width: isMobile ? '100%' : 400, maxWidth:'95vw', fontFamily:PX.sans,
          border:`1px solid ${PX.line}`, boxShadow:'0 30px 60px -20px rgba(19,42,79,.5)' }} onClick={e => e.stopPropagation()}>
        {isMobile && <div style={{ width:40, height:4, background:PX.line2, borderRadius:2, margin:'0 auto 16px' }}/>}
        <div style={{ width:40, height:40, borderRadius:12, marginBottom:12, display:'flex', alignItems:'center', justifyContent:'center',
          background: danger ? PX.badBg : PX.goldBg, color: danger ? PX.bad : PX.gold }}>
          {danger ? <PIcon.alert size={20}/> : <PIcon.shield size={20}/>}
        </div>
        <div style={{ fontFamily:PX.serif, fontSize:19, fontWeight:600, color:PX.ink, marginBottom:6 }}>{title}</div>
        <p style={{ fontSize:13.5, color:PX.sub, marginBottom:22, lineHeight:1.65 }}>{message}</p>
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onConfirm} className="px-btn" style={{ flex:1, minHeight:44, ...(danger ? { background:`linear-gradient(180deg,#d0342a,${PX.bad})` } : {}) }}>{confirmLabel}</button>
          <button onClick={onCancel} className="px-btn ghost" style={{ minHeight:44 }}>Cancel</button>
        </div>
      </div>
    </div>
  )
...
// Shared inline styles, on the portal's premiumUI palette (PX). Focus
// rings, fonts and the page canvas come from PremiumStyles (.px-root).
const S = {
  ...
  btn:    (color=PX.navy, disabled=false) => ({ display:'inline-flex', alignItems:'center', justifyContent:'center', gap:7, background: disabled ? PX.line2 : (color===PX.navy ? `linear-gradient(180deg,${PX.navy2},${PX.navy})` : color), color:'#fff', border:'none', borderRadius:11, padding:'10px 18px', fontWeight:700, cursor: disabled?'not-allowed':'pointer', fontSize:13.5, minHeight:44, boxShadow: disabled ? 'none' : '0 6px 14px -8px rgba(19,42,79,.6)', whiteSpace:'nowrap' }),
  btnSm:  (color=PX.navy) => ({ display:'inline-flex', alignItems:'center', gap:5, backgroundColor:color, color:'#fff', border:'none', borderRadius:9, padding:'6px 12px', fontWeight:600, cursor:'pointer', fontSize:12, minHeight:36, whiteSpace:'nowrap' }),
  input:  { width:'100%', padding:'10px 13px', borderRadius:11, border:`1px solid ${PX.line}`, fontSize:13.5, boxSizing:'border-box', background:'#fff', color:PX.ink, minHeight:44 },
  label:  { display:'block', fontSize:10.5, fontWeight:700, color:PX.sub, marginBottom:6, textTransform:'uppercase', letterSpacing:'.08em' },
  select: { width:'100%', padding:'10px 13px', borderRadius:11, border:`1px solid ${PX.line}`, fontSize:13.5, boxSizing:'border-box', background:'#fff', color:PX.ink, minHeight:44 },
  statCard: (color, bg) => ({ background:'#fff', borderRadius:16, padding:16, border:`1px solid ${PX.line}`, borderTop:`3px solid ${color}`, boxShadow:'0 1px 2px rgba(19,42,79,.05), 0 10px 26px -20px rgba(19,42,79,.35)', backgroundImage:`linear-gradient(180deg, ${bg}55, #fff 70%)` }),
  badge:  (color, bg) => ({ padding:'3px 9px', borderRadius:999, fontSize:11, fontWeight:700, background:bg, color }),
  // Secondary action: white with a hairline border (premiumUI "ghost").
  ghostSm: (color=PX.ink2) => ({ display:'inline-flex', alignItems:'center', justifyContent:'center', gap:5, background:'#fff', color, border:`1px solid ${PX.line2}`, borderRadius:9, padding:'6px 12px', fontWeight:600, cursor:'pointer', fontSize:12, minHeight:36, whiteSpace:'nowrap' }),
...
// Module-scoped rules only (everything under .px-root). The old version
// restyled the whole page's <body>, fonts and scrollbars, and nested its
// mobile rules inside a stray @media block.
const globalCSS = `
  @keyframes tchToastIn { from { transform:translate(-50%,16px);opacity:0 } to { transform:translate(-50%,0);opacity:1 } }
  .px-root ::-webkit-scrollbar { width:6px; height:6px }
  ...
  return (
    <form onSubmit={onSubmit}>
      <div className="form-grid" style={S.formGrid}>
        ...
        <div style={{ gridColumn:'1/-1' }}>
          <label style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer', padding:'12px 14px', borderRadius:8, background: form.needs_doubt_session?'#fef9c3':'#faf8f3', border:`1px solid ${form.needs_doubt_session?'#f59e0b':'#e8e3d8'}`, minHeight:44 }}>
            ...
            <span style={{ fontWeight:600, fontSize:14, color: form.needs_doubt_session?'#b45309':'#2e3b52' }}>🔁 Needs Doubt Session</span>
      ...
      <div style={{ display:'flex', gap:10, marginTop:14, flexWrap:'wrap' }}>
        <button type="submit" disabled={saving} style={S.btn(editMode?'#a7771f':'#132a4f', saving)}>
          {saving ? '⏳ Saving...' : editMode ? '✏️ Update Log' : '✅ Save Log'}
        </button>
        {onCancel && <button type="button" onClick={onCancel} style={S.btn('#5d6b82')}>✖ Cancel</button>}
      </div>
    </form>
  )
}
...
useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="stat-grid-4" style={S.statGrid(4)}>
        {[
          { label:'Total Logs',  value:logs.length,     color:'#132a4f', bg:'#eef2f9', icon:'📋' },
          { label:'Today Logs',  value:todayCount,       color:'#16a34a', bg:'#dcfce7', icon:'📅' },
          { label:'Subjects',    value:[...new Set(logs.map(l=>l.subject_name).filter(Boolean))].length, color:'#a7771f', bg:'#fbf3e0', icon:'📚' },
      ...
      <div style={S.card}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: showForm?16:0, flexWrap:'wrap', gap:8 }}>
          <h2 style={{ fontSize:16, fontWeight:800, color:'#132a4f', margin:0 }}>➕ Add Teaching Log</h2>
          <button onClick={() => { setShowForm(!showForm); setDupWarn(''); setDupBlocked(false) }} style={S.btn(showForm?'#5d6b82':'#132a4f')}>
  ...
  />
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:12 }}>
        ...
        <button onClick={exportCSV} style={{ ...S.ghostSm(), minHeight:44 }}><PIcon.download size={14}/> CSV</button>
        <button onClick={() => window.print()} style={{ ...S.ghostSm(), minHeight:44 }}><PIcon.print size={14}/> Print</button>
      ...
      <div style={{ fontSize:12, color:'#5d6b82', marginBottom:8 }}>
  ...
  {paginated.map((item, i) => {
    ...
    return (
      <div key={item.id} style={{
        ...
        border: doubtOpen ? '1.5px solid #f59e0b' : editId===item.id ? '2px solid #a7771f' : '1.5px solid #e8e3d8',
        ...
        <div style={{
          ...
          background: doubtOpen
            ...
            : editId===item.id
            ? 'linear-gradient(90deg,#a7771f,#e2c57e)'
            : `linear-gradient(90deg,${PX.navy},${PX.gold})`,
        }}/>
        ...
        <div style={{ padding:'16px 18px' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:4 }}>
                <span style={{ fontSize:15, fontWeight:800, color:'#14213d' }}>{item.subject_name}</span>
                {hasDoubt && (
              ...
              <div style={{ fontSize:12, color:'#5d6b82' }}>
                <span style={S.badge('#132a4f','#eef2f9')}>{item.course||'-'}</span>
                <span style={{ margin:'0 5px', color:'#d9d2c2' }}>·</span>
                <span style={{ fontWeight:600 }}>{item.subtype||'-'}</span>
                {item.class_name && <><span style={{ margin:'0 5px', color:'#d9d2c2' }}>·</span><span>{item.class_name}</span></>}
              </div>
            </div>
            <div style={{ textAlign:'right', flexShrink:0, marginLeft:8 }}>
              <div style={{ fontSize:11, fontWeight:700, color:'#132a4f', background:'#eef2f9', padding:'3px 10px', borderRadius:999 }}>
                {fmtDate(item.teaching_date)}
              </div>
              {item.period_number && (
                <div style={{ fontSize:10, color:'#8a93a6', marginTop:3 }}>P{item.period_number}</div>
              )}
            </div>
          </div>

          {/* ── Teacher ── */}
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10, padding:'8px 12px', background:'#faf8f3', borderRadius:8 }}>
            <div style={{ width:30, height:30, borderRadius:'50%', background:`linear-gradient(135deg,${PX.navy},${PX.navy2})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, color:PX.goldLt, fontWeight:800, flexShrink:0 }}>
              {(item.teacher_name||'?')[0].toUpperCase()}
            </div>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:'#14213d' }}>{item.teacher_name||'—'}</div>
              <div style={{ fontSize:10, color:'#8a93a6' }}>Subject Teacher</div>
            </div>
          </div>
          ...
          {item.topic_taught && (
            <div style={{ marginBottom:8 }}>
              <div style={{ fontSize:10, fontWeight:700, color:'#8a93a6', textTransform:'uppercase', letterSpacing:'.07em', marginBottom:3 }}>Topic Taught</div>
              <div style={{ fontSize:13, color:'#2e3b52', lineHeight:1.5, fontWeight:500 }}>{item.topic_taught}</div>
            </div>
          )}
          ...
          {feedback.map((fb, fi) => (
            <div key={fi} style={{ marginBottom:6, padding:'7px 10px', background:'#f0f9ff', border:'1px solid #bae6fd', borderRadius:7 }}>
              ...
              <span style={{ fontSize:10, color:'#8a93a6', marginLeft:6 }}>— {fb.hm_name}</span>
            </div>
          ))}
          ...
          {sessions[item.id]?.map(s => (
            <div key={s.id} style={{ marginBottom:6, padding:'8px 10px', background:s.status==='resolved'?'#f0fdf4':'#fffbeb', border:`1px solid ${s.status==='resolved'?'#bbf7d0':'#fde68a'}`, borderRadius:7 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:4 }}>
                <div>
                  <span style={{ fontSize:11, fontWeight:700, color:'#14213d' }}>🏠 {s.house_name||s.batch_name||'—'}</span>
                  <span style={{ fontSize:10, color:'#5d6b82', marginLeft:6 }}>HM: {s.hm_name||s.staff_name||'—'}</span>
                </div>
              ...
              {s.status==='open' && (
                <div style={{ display:'flex', gap:6, marginTop:6, alignItems:'center' }}>
                  <input
                    ...
                    style={{ flex:1, padding:'5px 8px', borderRadius:6, border:'1px solid #d9d2c2', fontSize:11, minHeight:32 }}
              ...
              {s.resolution_note && <div style={{ fontSize:11, color:'#5d6b82', marginTop:4 }}>📝 {s.resolution_note}</div>}
            </div>
          ))}

          {/* ── Edit form ── */}
          {editId===item.id && (
            <div style={{ marginTop:12, borderTop:'1px solid #e8e3d8', paddingTop:12 }}>
              <div style={{ fontSize:12, fontWeight:700, color:'#a7771f', marginBottom:10 }}>✏️ Edit Log</div>
          ...
          {/* ── Actions ── */}
          <div style={{ display:'flex', gap:6, marginTop:12, borderTop:'1px solid #f3f0e8', paddingTop:10 }}>
            <button onClick={() => editId===item.id ? (setEditId(null), setEditForm(null)) : startEdit(item)}
              style={{ ...S.ghostSm(editId===item.id ? PX.sub : PX.navy), flex:1 }}>
              {editId===item.id ? '✖ Cancel' : <><PIcon.pen size={13}/> Edit</>}
            </button>
            <button onClick={() => printLog(item)} style={{ ...S.ghostSm(), flex:1 }}><PIcon.print size={13}/> Print</button>
  ...
  {filtered.length===0 && (
    <div style={{ gridColumn:'1/-1', textAlign:'center', padding:48, color:'#8a93a6', background:'white', borderRadius:16 }}>
      No teaching logs found
              ...
              <div style={{ display:'flex', justifyContent:'center', gap:6, marginTop:14, flexWrap:'wrap' }}>
                <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1} style={{ ...S.btnSm('#5d6b82'), opacity:page===1?.4:1 }}>←</button>
                {Array.from({ length:Math.min(5,totalPages) }, (_,i) => {
                  const p = totalPages<=5?i+1:Math.max(1,Math.min(page-2,totalPages-4))+i
                  return <button key={p} onClick={() => setPage(p)} style={{ ...S.btnSm(page===p?'#132a4f':'#e8e3d8'), color:page===p?'white':'#2e3b52', minWidth:36 }}>{p}</button>
                })}
                <button onClick={() => setPage(p => Math.min(totalPages,p+1))} disabled={page===totalPages} style={{ ...S.btnSm('#5d6b82'), opacity:page===totalPages?.4:1 }}>→</button>
              </div>
            )}
          </>
        )}
...
const SUBJECT_COLORS = ['#132a4f','#a7771f','#0891b2','#16a34a','#ca8a04','#dc2626','#c026d3','#0d9488']
...
function TabCalendar({ logs, missed }) {
  ...
  return (
    <>
      <div className="stat-grid-4" style={S.statGrid(4)}>
        {[
          { label:'Logs this month', value:monthTotal,  color:'#132a4f', bg:'#eef2f9', icon:'📋' },
          { label:'Active days',     value:activeDays,  color:'#16a34a', bg:'#dcfce7', icon:'📅' },
          { label:'Subjects taught', value:subjectCount,color:'#a7771f', bg:'#fbf3e0', icon:'📚' },
          { label:'Missed classes',  value:monthMissed, color:'#dc2626', bg:'#fee2e2', icon:'❌' },
        ].map(c => (
          <div key={c.label} style={S.statCard(c.color, c.bg)}>
            <div style={{ fontSize:18, marginBottom:4 }}>{c.icon}</div>
            <p style={{ fontSize:12, color:c.color, fontWeight:700, margin:0 }}>{c.label}</p>
            <h2 style={{ fontSize:24, fontWeight:800, color:c.color, margin:'2px 0 0', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{c.value}</h2>
          </div>
        ))}
      </div>
      ...
      <div style={S.card}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14, flexWrap:'wrap', gap:10 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
            <button onClick={prevMonth} style={S.btnSm('#5d6b82')}>◀</button>
            <span style={{ fontSize:16, fontWeight:800, color:'#132a4f', minWidth:140, textAlign:'center' }}>
              {new Date(year,mon-1).toLocaleString('default',{month:'long'})} {year}
            </span>
            <button onClick={nextMonth} style={S.btnSm('#5d6b82')}>▶</button>
            <input type="month" value={month} onChange={e => { setMonth(e.target.value); setSelectedDay(null) }} style={{ padding:'6px 10px', borderRadius:8, border:'1px solid #d9d2c2', fontSize:13, minHeight:36 }}/>
          </div>
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            ...
            <button onClick={() => setViewMode('month')} style={{ ...S.btnSm(viewMode==='month'?'#132a4f':'#e8e3d8'), color:viewMode==='month'?'white':'#2e3b52' }}>Month</button>
            <button onClick={() => setViewMode('week')}  style={{ ...S.btnSm(viewMode==='week' ?'#132a4f':'#e8e3d8'), color:viewMode==='week' ?'white':'#2e3b52' }}>Week</button>
          </div>
        </div>
          ...
          <>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:2, marginBottom:2 }}>
              {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d => (
                <div key={d} style={{ textAlign:'center', fontSize:10, fontWeight:700, color:'#8a93a6', padding:4 }}>{d}</div>
              ))}
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:2 }}>
              ...
              {Array.from({ length:daysInMonth }).map((_,i) => {
                ...
                const bg    = isSel?'#132a4f':isToday?'#e0f2fe':hasLogs?'#dcfce7':hasMissed?'#fee2e2':'#faf8f3'
                const color = isSel?'white':isToday?'#0891b2':hasLogs?'#15803d':hasMissed?'#dc2626':'#8a93a6'
                const dayLogs = logsByDate[day]||[]
                return (
                  <div key={day} onClick={() => setSelectedDay(day===selectedDay?null:day)}
                    style={{ background:bg, border:`1px solid ${isSel?'#132a4f':isToday?'#7dd3fc':hasLogs?'#bbf7d0':hasMissed?'#fecaca':'#e8e3d8'}`, borderRadius:8, padding:'5px 3px', textAlign:'center', cursor:'pointer', minHeight:52 }}>
                    ...
                    {hasLogs && (
                      <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'center', gap:2, marginTop:3 }}>
                        {dayLogs.slice(0,4).map((l,j) => (
                          <span key={j} title={l.subject_name} style={{ width:6, height:6, borderRadius:'50%', background: isSel?'rgba(255,255,255,0.7)':subjectColorMap[l.subject_name]||'#8a93a6', display:'inline-block' }}/>
                        ))}
                        {dayLogs.length>4 && <span style={{ fontSize:8, color:isSel?'#d9d2c2':color }}>+{dayLogs.length-4}</span>}
                      </div>
                    )}
          ...
          <div className="table-wrap">
            <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:8, minWidth:480 }}>
              {weekDays.map(dateStr => {
                ...
                const isToday = dateStr===today()
                return (
                  <div key={dateStr} style={{ border:`2px solid ${isToday?'#132a4f':'#e8e3d8'}`, borderRadius:10, overflow:'hidden' }}>
                    <div style={{ padding:'7px 8px', background: isToday?'#132a4f':'#faf8f3', textAlign:'center' }}>
                      <div style={{ fontSize:10, fontWeight:600, color:isToday?'#b7c6e0':'#8a93a6' }}>{new Date(dateStr).toLocaleString('default',{weekday:'short'})}</div>
                      <div style={{ fontSize:15, fontWeight:800, color:isToday?'white':'#14213d' }}>{dayNum}</div>
                    </div>
                    <div style={{ padding:5, minHeight:70 }}>
                      {dayLogs.length===0 && <div style={{ textAlign:'center', color:'#e8e3d8', fontSize:10, paddingTop:8 }}>—</div>}
                      {dayLogs.map((l,j) => (
                        <div key={j} style={{ marginBottom:3, padding:'3px 5px', borderRadius:4, background:`${subjectColorMap[l.subject_name]||'#132a4f'}18`, borderLeft:`3px solid ${subjectColorMap[l.subject_name]||'#132a4f'}` }}>
                          <div style={{ fontSize:10, fontWeight:700, color:subjectColorMap[l.subject_name]||'#132a4f' }}>{l.subject_name}</div>
                          <div style={{ fontSize:9, color:'#5d6b82' }}>{l.teacher_name||'-'}</div>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {selectedDay && viewMode==='month' && (
          <div style={{ marginTop:18, borderTop:'1px solid #e8e3d8', paddingTop:18 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              <h3 style={{ fontSize:14, fontWeight:800, color:'#132a4f', margin:0 }}>
                {selectedDay} {new Date(year,mon-1).toLocaleString('default',{month:'long'})} {year}
              </h3>
              <span style={{ fontSize:12, color:'#5d6b82' }}>{selectedLogs.length} log{selectedLogs.length!==1?'s':''}</span>
            </div>
            {selectedLogs.length===0 && selectedMissed.length===0 && <p style={{ color:'#8a93a6', fontSize:13 }}>No activity recorded.</p>}
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))', gap:10 }}>
              {selectedLogs.map(l => (
                <div key={l.id} style={{ background:'#f0fdf4', borderLeft:`4px solid ${subjectColorMap[l.subject_name]||'#16a34a'}`, borderRadius:8, padding:'12px 14px' }}>
                  ...
                  <div style={{ fontSize:12, color:'#2e3b52', marginTop:3, fontStyle:'italic' }}>{l.topic_taught}</div>
                  <div style={{ fontSize:11, color:'#5d6b82', marginTop:4 }}>👨‍🏫 {l.teacher_name||'-'} · {l.subtype||l.course}</div>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div style={S.card}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18, flexWrap:'wrap', gap:10 }} className="no-print">
        <h2 style={{ fontSize:16, fontWeight:800, color:'#132a4f', margin:0 }}>📈 Monthly Report</h2>
        <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
          <input type="month" value={month} onChange={e => setMonth(e.target.value)} style={{ padding:'8px 12px', borderRadius:8, border:'1px solid #d9d2c2', fontSize:13, minHeight:44 }}/>
          ...
          <button onClick={handlePrint} style={S.btn('#a7771f')}>🖨️ Print</button>
        </div>
      </div>
      <div id="print-report">
        <div className="stat-grid-4" style={S.statGrid(4)}>
          {[
            { label:'Classes Taken',     value:monthLogs.length,    color:'#132a4f', bg:'#eef2f9' },
            { label:'Missed',            value:monthMissed.length,   color:'#dc2626', bg:'#fee2e2' },
            { label:'Subjects Covered',  value:new Set(monthLogs.map(l=>l.subject_name)).size, color:'#a7771f', bg:'#fbf3e0' },
            ...
            <div key={c.label} style={{ background:c.bg, borderRadius:10, padding:14, borderLeft:`4px solid ${c.color}` }}>
              ...
              <div style={{ fontSize:24, fontWeight:800, color:c.color, marginTop:4, fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{c.value}</div>
            </div>
          ))}
        </div>
        {Object.entries(byTeacher).map(([name, data]) => {
          ...
          return (
            <div key={name} style={{ border:'1px solid #e8e3d8', borderRadius:10, padding:16, marginBottom:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10, flexWrap:'wrap', gap:6 }}>
                <span style={{ fontWeight:800, color:'#14213d', fontSize:14 }}>👨‍🏫 {name}</span>
                <span style={{ fontSize:12, color:'#5d6b82' }}>{data.logs.length} classes · {data.subjects.size} subjects · {avgPerDay} avg/day</span>
              </div>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:8 }}>{[...data.subjects].map(s=><span key={s} style={S.pill('#132a4f','#eef2f9')}>{s}</span>)}</div>
              {[...data.logs].slice(0, expanded[name] ? data.logs.length : 5).map(l => (
                <div key={l.id} style={{ borderBottom:'1px solid #f3f0e8', padding:'4px 0', fontSize:13, color:'#5d6b82' }}>
              ...
              {data.logs.length>5 && (
                <button
                  ...
                  className="no-print"
                  style={{
                    color:'#132a4f', fontSize:12, marginTop:6, fontWeight:700,
                    background:'#eef2f9', border:'none', borderRadius:7,
        ...
        {Object.keys(byTeacher).length===0 && <div style={{ textAlign:'center', padding:32, color:'#8a93a6' }}>No logs for this period.</div>}
      </div>
    </div>
  )
}
...
function TabStudentPerformance({ courseData, logs, currentUser }) {
  ...
  const printFilteredTable = () => {
    ...
    win.document.write(`<html><head><title>Class Test Scores</title><style>
      body{font-family:Georgia,serif;padding:32px;color:#14213d} h1{font-size:20px;margin:0 0 16px}
      table{width:100%;border-collapse:collapse;font-size:12px} th,td{padding:6px 8px;text-align:left;border-bottom:1px solid #e8e3d8} th{background:#faf8f3}
      @media print{body{padding:12px}}
    </style></head><body>
  ...
  return (
    <>
      ...
      <div className="stat-grid-4" style={S.statGrid(4)}>
        {[
          { label:'Total Assessments', value:filtered.length, color:'#132a4f', bg:'#eef2f9', icon:'📝' },
      ...
      <div style={{ position:'relative', marginBottom:14 }}>
        ...
        {searchResults.length > 0 && (
          <div style={{ position:'absolute', top:'100%', left:0, marginTop:4, width:340, maxWidth:'100%', background:'white', border:'1px solid #e8e3d8', borderRadius:8, boxShadow:'0 6px 20px rgba(0,0,0,.12)', zIndex:20, overflow:'hidden' }}>
            {searchResults.map(s => (
              <div key={s.id||s.name} onClick={() => { setFilterStudent(s); setStudentSearch(''); setViewMode('trend') }} style={{ padding:'10px 14px', cursor:'pointer', fontSize:13, borderBottom:'1px solid #f3f0e8' }} onMouseDown={e=>e.preventDefault()}>
                {s.name}
              </div>
            ))}
          </div>
      ...
      <div style={S.card}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: (showForm||bulkMode)?18:0, flexWrap:'wrap', gap:8 }}>
          <h2 style={{ fontSize:16, fontWeight:800, color:'#132a4f', margin:0 }}>🎯 {editingId?'Edit Score':'Add Score'}</h2>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            <button onClick={() => csvFileRef.current?.click()} style={{ ...S.ghostSm(), minHeight:44, padding:'10px 16px', fontSize:13.5 }}><PIcon.download size={15}/> Import CSV</button>
            <input ref={csvFileRef} type="file" accept=".csv" onChange={handleCsvSelect} style={{ display:'none' }}/>
            <button onClick={() => { setBulkMode(!bulkMode); setShowForm(false); setEditingId(null); setLastSavedBatch(null) }} style={{ ...S.ghostSm(bulkMode ? PX.sub : PX.ink2), minHeight:44, padding:'10px 16px', fontSize:13.5 }}>{bulkMode ? '✖ Cancel' : <><PIcon.list size={15}/> Bulk entry</>}</button>
            <button onClick={() => { setShowForm(!showForm); setBulkMode(false); setEditingId(null); setForm(blankForm) }} style={S.btn(showForm?'#5d6b82':'#132a4f')}>{showForm?'✖ Cancel':'➕ Add Score'}</button>
          </div>
        </div>
        ...
        {bulkMode && (
          <div>
            <div style={{ padding:'10px 14px', background:'#fbf3e0', border:'1px solid #eadbb2', borderRadius:8, marginBottom:14, fontSize:13, color:'#a7771f', fontWeight:600 }}>
            ...
            {bulkStudents.length > 0 && (
              <>
                <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', marginBottom:10 }}>
                  📝 Enter marks for {bulkStudents.length} students — Max: {bulkForm.max_score}
                  <span style={{ marginLeft:12, fontSize:12, color:'#8a93a6', fontWeight:400 }}>Leave blank to skip a student</span>
                </div>
                <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:10 }}>
                  ...
                  <button onClick={() => { const m = {}; bulkStudents.forEach(s => { m[s.id] = '' }); setBulkMarks(m) }} style={S.btnSm('#8a93a6')}>✕ Clear All</button>
                </div>
                <div style={{ border:'1px solid #e8e3d8', borderRadius:10, overflow:'hidden', marginBottom:14 }}>
                  <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                    <thead>
                      <tr style={{ background:'#faf8f3', borderBottom:'1px solid #e8e3d8' }}>
                        <th style={{ padding:'10px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:12 }}>#</th>
                        <th style={{ padding:'10px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:12 }}>Student</th>
                        <th style={{ padding:'10px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:12 }}>Marks / {bulkForm.max_score}</th>
                        <th style={{ padding:'10px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:12 }}>%</th>
                        <th style={{ padding:'10px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:12 }}>Grade</th>
                      </tr>
                    </thead>
                    <tbody>
                      {bulkStudents.map((s, i) => {
                        ...
                        return (
                          <tr key={s.id} style={{ borderBottom:'1px solid #f3f0e8', background: !hasName ? '#fef2f2' : p!==null?(p>=75?'#f0fdf4':p>=50?'#fffbeb':'#fff1f2'):'white' }}>
                            <td style={{ padding:'8px 12px', color:'#8a93a6', fontSize:11 }}>{i+1}</td>
                            <td style={{ padding:'8px 12px', fontWeight:600, color: hasName ? '#14213d' : '#dc2626' }}>
                              {hasName ? s.name : '⚠️ No name on file'}
                              {s.roll_number && <span style={{ marginLeft:6, fontSize:11, color:'#8a93a6' }}>#{s.roll_number}</span>}
                            </td>
                            <td style={{ padding:'8px 12px' }}>
                              <input
                                ...
                                style={{ width:80, padding:'6px 10px', borderRadius:6, border:`1.5px solid ${p!==null?scoreColor(p):'#d9d2c2'}`, fontSize:13, fontFamily:'inherit', textAlign:'center', opacity: hasName?1:.5 }}
                              />
                            </td>
                            <td style={{ padding:'8px 12px', fontWeight:700, color:p!==null?scoreColor(p):'#8a93a6', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>
                              {p !== null ? `${p}%` : '—'}
                            </td>
                            <td style={{ padding:'8px 12px' }}>
                ...
                <div style={{ maxHeight:200, overflowY:'auto' }}>
                  {lastSavedBatch.map((s,i) => (
                    <div key={s.id||i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 8px', fontSize:12, borderBottom:'1px solid #dcfce7' }}>
                      <span style={{ color:'#14213d' }}>{s.student_name}</span>
                      <span style={{ fontWeight:700, color:'#16a34a', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{s.score}/{s.max_score}</span>
                    </div>
                  ))}
                </div>
                <button onClick={() => setLastSavedBatch(null)} style={{ ...S.btnSm('#8a93a6'), marginTop:8 }}>Dismiss</button>
              </div>
            )}
          </div>
        )}
        ...
        {csvPreview && (
          <div style={{ marginTop:16, padding:14, background:'#ecfeff', border:'1px solid #a5f3fc', borderRadius:10 }}>
            <div style={{ fontSize:13, fontWeight:700, color:'#0891b2', marginBottom:8 }}>
              ...
              <span style={{ marginLeft:10, fontSize:11, fontWeight:400, color:'#5d6b82' }}>Expected columns: student_name, subject_name, topic, test_date, score, max_score, subtype, course, class_name</span>
            </div>
            ...
            <div style={{ display:'flex', gap:10 }}>
              ...
              <button onClick={() => setCsvPreview(null)} style={S.btn('#5d6b82')}>✖ Cancel</button>
            </div>
          </div>
        )}
        {showForm && (
          <form onSubmit={handleSave} className="form-grid" style={{ ...S.formGrid, marginTop:16 }}>
            ...
            <div style={{ gridColumn:'1/-1', display:'flex', gap:10 }}>
              <button type="submit" disabled={saving} style={S.btn('#132a4f',saving)}>{saving?'⏳ Saving...':'✅ Save Score'}</button>
              <button type="button" onClick={() => { setShowForm(false); setEditingId(null); setForm(blankForm) }} style={S.btn('#5d6b82')}>✖ Cancel</button>
            </div>
          </form>
        )}
      </div>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:14 }}>
        ...
        <div style={{ display:'flex', gap:4, marginLeft:'auto' }}>
          ...
          <button title="Print / PDF" onClick={printFilteredTable} style={{ ...S.btnSm('#a7771f'), color:'white' }}>🖨️</button>
        </div>
      </div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, background:'white', minWidth:760 }}>
            <thead><tr style={{ background:'#faf8f3', borderBottom:'1px solid #e8e3d8' }}>
              {[['date','Date'],['student','Student'],['batch','Batch'],['subject','Subject'],['topic','Topic'],['score','Score'],['pct','%'],['rank','Rank'],['trend','Trend'],['grade','Grade'],['actions','Actions']].map(([col,h]) => {
                ...
                return (
                  <th key={col} onClick={() => sortable && toggleSort(col)} style={{ padding:'11px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:12, cursor:sortable?'pointer':'default', userSelect:'none' }}>
            ...
            <tbody>
              {sortedFiltered.map(s => { const p = pct(s.score, s.max_score); const r = rankMap[s.id]; const key = studentKey(s); const trend = studentTrend[key]; return (
                <tr key={s.id} style={{ borderBottom:'1px solid #f3f0e8' }}>
                  <td style={{ padding:'9px 12px', color:'#5d6b82', whiteSpace:'nowrap' }}>{fmtDate(s.test_date)}</td>
                  <td style={{ padding:'9px 12px', fontWeight:600, color:'#14213d' }}>{s.student_name}</td>
                  <td style={{ padding:'9px 12px' }}><span style={S.badge('#132a4f','#eef2f9')}>{s.subtype||s.course||'-'}</span></td>
                  <td style={{ padding:'9px 12px', color:'#2e3b52' }}>{s.subject_name}</td>
                  <td style={{ padding:'9px 12px', color:'#5d6b82', maxWidth:140 }}>{s.topic}</td>
                  <td style={{ padding:'9px 12px', fontWeight:700, color:'#14213d', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{s.score}/{s.max_score}</td>
                  <td style={{ padding:'9px 12px' }}><div style={{ display:'flex', alignItems:'center', gap:6 }}><div style={{ width:44, height:5, background:'#e8e3d8', borderRadius:3, overflow:'hidden' }}><div style={{ width:`${p}%`, height:'100%', background:scoreColor(p), borderRadius:3 }}/></div><span style={{ fontWeight:700, color:scoreColor(p), fontSize:12, fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{p}%</span></div></td>
                  <td style={{ padding:'9px 12px', fontSize:12, color:'#5d6b82' }}>{r ? `#${r.rank}/${r.outOf}` : '-'}</td>
                  ...
                  <td style={{ padding:'9px 12px' }}><div style={{ display:'flex', gap:5 }}><button onClick={() => startEdit(s)} style={S.btnSm('#a7771f')}>✏️</button><button title="Share on WhatsApp" onClick={() => shareWhatsApp(key)} style={S.btnSm('#16a34a')}>💬</button>{isAdmin && <button onClick={() => setConfirmDel(s.id)} style={S.btnSm('#dc2626')}>🗑</button>}</div></td>
                </tr>
              )})}
              {filtered.length===0 && <tr><td colSpan={11} style={{ padding:32, textAlign:'center', color:'#8a93a6' }}>No score data.</td></tr>}
            </tbody>
          </table>
        </div>
      ))}
        ...
        <div style={S.card}>
          ...
          {weakOnly.length===0?<div style={{ textAlign:'center', padding:32, color:'#16a34a', fontWeight:600 }}>✅ No weak areas detected.</div>:weakOnly.map((w,i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:14, padding:'12px 14px', border:'1px solid #fecaca', borderRadius:10, marginBottom:8, background:'#fff1f2', flexWrap:'wrap' }}>
              <div style={{ minWidth:130 }}><div style={{ fontWeight:700, color:'#14213d', fontSize:13 }}>{w.student}</div><div style={{ fontSize:12, color:'#5d6b82' }}>{w.batch}</div></div>
              <div style={{ flex:1, minWidth:100 }}><div style={{ fontSize:12, color:'#2e3b52', marginBottom:4 }}>{w.subject}</div><div style={{ height:7, background:'#fee2e2', borderRadius:3, overflow:'hidden' }}><div style={{ width:`${w.avg}%`, height:'100%', background:'#dc2626', borderRadius:3 }}/></div></div>
              <span style={{ fontWeight:800, color:'#dc2626', fontSize:18, fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{w.avg}%</span>
            </div>
          ))}
        </div>
      )}
      ...
      {viewMode==='trend' && (
        <div style={S.card}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:8, marginBottom:0 }}>
            <h3 style={{ fontSize:15, fontWeight:800, color:'#132a4f', margin:0 }}>📈 Score Trend — {filterStudent==='All'?'Select a student above':filterStudent.name}</h3>
            {filterStudent!=='All' && trendData.length>0 && (
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={() => printStudentReport(filterStudent.id||filterStudent.name)} style={S.btnSm('#a7771f')}>🖨️ Print Report</button>
            ...
            <>
              <div style={{ display:'flex', gap:8, alignItems:'flex-end', height:160, padding:'0 6px', borderBottom:'2px solid #e8e3d8', overflowX:'auto', marginTop:14, marginBottom:14 }}>
                {trendData.map((s,i) => { const p = pct(s.score, s.max_score); return (
                  <div key={s.id} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3, minWidth:54 }}>
                    ...
                    <div style={{ fontSize:10, color:'#5d6b82', textAlign:'center', maxWidth:54, wordBreak:'break-word' }}>{s.subject_name?.slice(0,5)}</div>
                    <div style={{ fontSize:9, color:'#8a93a6' }}>{s.test_date?.slice(5)}</div>
                  </div>
                )})}
              </div>
              <div className="stat-grid-4" style={{ ...S.statGrid(4), marginBottom:14 }}>
                {[{label:'Tests',value:trendData.length},{label:'Best',value:`${Math.max(...trendData.map(s=>pct(s.score,s.max_score)))}%`},{label:'Latest',value:`${pct(trendData[trendData.length-1].score,trendData[trendData.length-1].max_score)}%`},{label:'Average',value:`${Math.round(trendData.reduce((a,s)=>a+pct(s.score,s.max_score),0)/trendData.length)}%`}].map(c=>(
                  <div key={c.label} style={{ background:'#faf8f3', borderRadius:8, padding:'12px 14px' }}><div style={{ fontSize:12, color:'#5d6b82' }}>{c.label}</div><div style={{ fontSize:20, fontWeight:800, color:'#132a4f', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{c.value}</div></div>
                ))}
              </div>
              ...
              {subjectComparisonForStudent.length > 1 && (
                <div style={{ marginBottom:16 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', marginBottom:8 }}>Subject Comparison</div>
                  {subjectComparisonForStudent.map(sc => (
                    <div key={sc.subject} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
                      <span style={{ width:120, fontSize:12, color:'#2e3b52', flexShrink:0 }}>{sc.subject}</span>
                      <div style={{ flex:1, height:8, background:'#f3f0e8', borderRadius:4, overflow:'hidden' }}><div style={{ width:`${sc.avg}%`, height:'100%', background:scoreColor(sc.avg), borderRadius:4 }}/></div>
              ...
              {[...trendData].reverse().map(s => { const p = pct(s.score, s.max_score); const g = classAvgByGroup[`${s.subject_name}||${s.subtype}`]; const delta = g!==undefined ? p-g : null; return (
                <div key={s.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'8px 12px', border:'1px solid #f3f0e8', borderRadius:8, marginBottom:5, fontSize:13, flexWrap:'wrap' }}>
                  <span style={{ color:'#8a93a6', minWidth:70 }}>{fmtDate(s.test_date)}</span>
                  <span style={{ flex:1, color:'#2e3b52' }}>{s.subject_name} — <em style={{ color:'#5d6b82' }}>{s.topic}</em></span>
                  <span style={{ fontWeight:700, color:scoreColor(p), fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{s.score}/{s.max_score} ({p}%)</span>
      ...
      {viewMode==='analytics' && (
        <div style={S.card}>
          <h3 style={{ fontSize:15, fontWeight:800, color:'#132a4f', marginTop:0 }}>🧮 Performance Analytics</h3>
          {filtered.length===0 ? <div style={{ textAlign:'center', padding:32, color:'#8a93a6' }}>No score data for current filters.</div> : (
            <>
              <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', margin:'4px 0 10px' }}>Subject-wise Average</div>
              <div style={{ marginBottom:20 }}>
                {analytics.subjectAverages.map(sa => (
                  <div key={sa.subject} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                    <span style={{ width:130, fontSize:12, color:'#2e3b52', flexShrink:0 }}>{sa.subject}</span>
                    <div style={{ flex:1, height:10, background:'#f3f0e8', borderRadius:5, overflow:'hidden' }}>
                      <div style={{ width:`${sa.avg}%`, height:'100%', background:scoreColor(sa.avg), borderRadius:5 }}/>
                    </div>
                    <span style={{ width:70, textAlign:'right', fontWeight:700, fontSize:12, color:scoreColor(sa.avg), fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{sa.avg}% ({sa.count})</span>
                  </div>
                ))}
              </div>

              <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', margin:'4px 0 10px' }}>Grade Distribution</div>
              <div className="stat-grid-4" style={{ ...S.statGrid(4), marginBottom:20 }}>
                {Object.entries(analytics.dist).map(([label,count]) => (
                  <div key={label} style={{ background:'#faf8f3', borderRadius:8, padding:'12px 14px', textAlign:'center' }}>
                    <div style={{ fontSize:11, color:'#5d6b82' }}>{label}</div>
                    <div style={{ fontSize:22, fontWeight:800, color:'#132a4f', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{count}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:16 }}>
                <div>
                  ...
                  {analytics.top5.length===0 ? <div style={{ fontSize:12, color:'#8a93a6' }}>No data.</div> : analytics.top5.map((s,i) => (
                    <div key={s.name+i} style={{ display:'flex', justifyContent:'space-between', padding:'8px 10px', borderRadius:8, background:'#f0fdf4', marginBottom:5, fontSize:13 }}>
                      <span style={{ fontWeight:600, color:'#14213d' }}>{i+1}. {s.name}</span>
                      <span style={{ fontWeight:700, color:'#16a34a', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{s.avg}% ({s.count})</span>
                    </div>
                  ))}
                </div>
                <div>
                  <div style={{ fontSize:13, fontWeight:700, color:'#dc2626', marginBottom:8 }}>📉 Needs Attention</div>
                  {analytics.bottom5.length===0 ? <div style={{ fontSize:12, color:'#8a93a6' }}>No data.</div> : analytics.bottom5.map((s,i) => (
                    <div key={s.name+i} style={{ display:'flex', justifyContent:'space-between', padding:'8px 10px', borderRadius:8, background:'#fff1f2', marginBottom:5, fontSize:13 }}>
                      <span style={{ fontWeight:600, color:'#14213d' }}>{i+1}. {s.name}</span>
                      <span style={{ fontWeight:700, color:'#dc2626', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{s.avg}% ({s.count})</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {viewMode==='insights' && (
        <div style={S.card}>
          <h3 style={{ fontSize:15, fontWeight:800, color:'#132a4f', marginTop:0 }}>💡 Deeper Insights</h3>
          {filtered.length===0 ? <div style={{ textAlign:'center', padding:32, color:'#8a93a6' }}>No score data for current filters.</div> : (
            <>
              <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:16, marginBottom:20 }}>
                <div>
                  <div style={{ fontSize:13, fontWeight:700, color:'#132a4f', marginBottom:8 }}>🚀 Most Improved (first → latest test)</div>
                  {mostImproved.filter(m=>m.delta>0).length===0 ? <div style={{ fontSize:12, color:'#8a93a6' }}>No students with 2+ tests showing improvement yet.</div> : mostImproved.filter(m=>m.delta>0).slice(0,6).map((m,i) => (
                    <div key={m.name+i} style={{ display:'flex', justifyContent:'space-between', padding:'8px 10px', borderRadius:8, background:'#eef2f9', marginBottom:5, fontSize:13 }}>
                      <span style={{ fontWeight:600, color:'#14213d' }}>{m.name}</span>
                      <span style={{ fontWeight:700, color:'#16a34a', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{m.first}% → {m.latest}% (+{m.delta})</span>
                    </div>
                  ))}
                </div>
                <div>
                  <div style={{ fontSize:13, fontWeight:700, color:'#132a4f', marginBottom:8 }}>🎯 Most Consistent (lowest variation)</div>
                  {consistency.length===0 ? <div style={{ fontSize:12, color:'#8a93a6' }}>Not enough data (needs 2+ tests per student).</div> : consistency.slice(0,6).map((c,i) => (
                    <div key={c.name+i} style={{ display:'flex', justifyContent:'space-between', padding:'8px 10px', borderRadius:8, background:'#faf8f3', marginBottom:5, fontSize:13 }}>
                      <span style={{ fontWeight:600, color:'#14213d' }}>{c.name}</span>
                      <span style={{ fontWeight:700, color:'#2e3b52', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>avg {c.avg}% · ±{c.stdDev}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', margin:'4px 0 10px' }}>Batch Comparison{filterSubject!=='All'?` — ${filterSubject}`:' — all subjects'}</div>
              {batchComparison.length===0 ? <div style={{ fontSize:12, color:'#8a93a6' }}>No data.</div> : batchComparison.map(b => (
                <div key={b.batch} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                  <span style={{ width:100, fontSize:12, color:'#2e3b52', flexShrink:0 }}>{b.batch}</span>
                  <div style={{ flex:1, height:10, background:'#f3f0e8', borderRadius:5, overflow:'hidden' }}>
      ...
      {viewMode==='tests' && (
        <div style={S.card}>
          <h3 style={{ fontSize:15, fontWeight:800, color:'#132a4f', marginTop:0 }}>🗂️ Test History</h3>
          {testHistory.length===0 ? <div style={{ textAlign:'center', padding:32, color:'#8a93a6' }}>No tests recorded yet.</div> : (
            <div style={{ maxHeight:480, overflowY:'auto' }}>
              {testHistory.map((t,i) => {
                const tkey = `${t.subject}||${t.topic}||${t.test_date}||${t.subtype}`
                return (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', border:'1px solid #f3f0e8', borderRadius:8, marginBottom:6, flexWrap:'wrap' }}>
                    <div style={{ flex:1, minWidth:180 }}>
                      <div style={{ fontWeight:700, fontSize:13, color:'#14213d' }}>{t.subject} — {t.topic}</div>
                      <div style={{ fontSize:11, color:'#5d6b82' }}>{t.subtype||'-'} · {fmtDate(t.test_date)} · {t.count} students</div>
                    </div>
                    <span style={{ fontWeight:700, fontSize:14, color:scoreColor(t.avg), fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{t.avg}%</span>
                    <button onClick={() => { setFilterBatch(t.subtype||'All'); setFilterSubject(t.subject); setViewMode('table') }} style={S.btnSm('#132a4f')}>🔎 View</button>
                    <button onClick={() => printBatchReport(tkey)} style={S.btnSm('#a7771f')}>🖨️</button>
      ...
      {viewMode==='classwise' && (
        <div style={S.card}>
          <h3 style={{ fontSize:15, fontWeight:800, color:'#132a4f', marginTop:0, marginBottom:14 }}>🏫 Class Wise Test</h3>
          ...
          {cwBatch==='All' ? (
            <div style={{ textAlign:'center', padding:32, color:'#8a93a6' }}>Select a class/batch above to see its test history.</div>
          ) : classWiseTests.length===0 ? (
            <div style={{ textAlign:'center', padding:32, color:'#8a93a6' }}>No tests recorded for {cwBatch}{cwSubject!=='All'?` — ${cwSubject}`:''} yet.</div>
          ) : (
            <>
              <div className="stat-grid-4" style={{ ...S.statGrid(4), marginBottom:18 }}>
                {[
                  { label:'Tests Conducted', value:classWiseTests.length, color:'#132a4f', bg:'#eef2f9' },
                  ...
                  <div key={c.label} style={S.statCard(c.color, c.bg)}>
                    ...
                    <h2 style={{ fontSize:20, fontWeight:800, color:c.color, margin:'2px 0 0', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{c.value}</h2>
                  </div>
                ))}
              </div>

              {/* Class average trend over time */}
              <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', marginBottom:8 }}>Class Average Trend</div>
              <div style={{ display:'flex', gap:8, alignItems:'flex-end', height:120, padding:'0 6px', borderBottom:'2px solid #e8e3d8', overflowX:'auto', marginBottom:20 }}>
                {classWiseTrend.map(t => (
                  <div key={t.key} title={`${t.subject} — ${t.topic}: ${t.avg}% (${t.count} students)`} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3, minWidth:46, cursor:'pointer' }} onClick={() => setCwTestKey(t.key)}>
                    <span style={{ fontSize:10, fontWeight:700, color:scoreColor(t.avg), fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{t.avg}%</span>
                    <div style={{ width:30, height:`${Math.max(t.avg,4)}px`, background: cwTestKey===t.key ? '#a7771f' : scoreColor(t.avg), borderRadius:'4px 4px 0 0', transition:'height .3s' }}/>
                    <div style={{ fontSize:9, color:'#8a93a6' }}>{t.test_date?.slice(5)}</div>
                  </div>
                ))}
              </div>
              ...
              {cwSubject==='All' && classWiseSubjectBreakdown.length > 1 && (
                <>
                  <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', marginBottom:8 }}>Subject Breakdown — {cwBatch}</div>
                  <div style={{ marginBottom:20 }}>
                    {classWiseSubjectBreakdown.map(sb => (
                      <div key={sb.subject} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                        <span style={{ width:130, fontSize:12, color:'#2e3b52', flexShrink:0 }}>{sb.subject}</span>
                        <div style={{ flex:1, height:10, background:'#f3f0e8', borderRadius:5, overflow:'hidden' }}>
              ...
              <div style={{ fontSize:13, fontWeight:700, color:'#2e3b52', marginBottom:8 }}>Tests for {cwBatch}{cwSubject!=='All'?` — ${cwSubject}`:''}</div>
              <div style={{ marginBottom: selectedClassTest ? 20 : 0 }}>
                {classWiseTests.map(t => (
                  <div key={t.key} onClick={() => setCwTestKey(cwTestKey===t.key?null:t.key)} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', border: cwTestKey===t.key?'1.5px solid #a7771f':'1px solid #f3f0e8', background: cwTestKey===t.key?'#fbf3e0':'white', borderRadius:8, marginBottom:6, cursor:'pointer', flexWrap:'wrap' }}>
                    <div style={{ flex:1, minWidth:180 }}>
                      <div style={{ fontWeight:700, fontSize:13, color:'#14213d' }}>{t.subject} — {t.topic}</div>
                      <div style={{ fontSize:11, color:'#5d6b82' }}>{fmtDate(t.test_date)} · {t.count} students · best {t.best}% · lowest {t.worst}%</div>
                    </div>
                    <span style={{ fontWeight:700, fontSize:15, color:scoreColor(t.avg), fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{t.avg}%</span>
                    <button onClick={e => { e.stopPropagation(); printBatchReport(t.key) }} style={S.btnSm('#a7771f')}>🖨️</button>
              ...
              {selectedClassTest && (
                <div style={{ border:'1px solid #e8e3d8', borderRadius:10, overflow:'hidden' }}>
                  <div style={{ padding:'10px 14px', background:'#fbf3e0', borderBottom:'1px solid #e8e3d8', fontSize:13, fontWeight:700, color:'#a7771f' }}>
                    📋 {selectedClassTest.subject} — {selectedClassTest.topic} ({fmtDate(selectedClassTest.test_date)}) — full roster
                  </div>
                  <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                    <thead><tr style={{ background:'#faf8f3' }}>{['Rank','Student','Score','%'].map(h=><th key={h} style={{ padding:'8px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:12 }}>{h}</th>)}</tr></thead>
                    <tbody>
                      {[...selectedClassTest.rows].sort((a,b) => pct(b.score,b.max_score)-pct(a.score,a.max_score)).map((r,i) => { const p = pct(r.score,r.max_score); return (
                        <tr key={r.id} style={{ borderTop:'1px solid #f3f0e8' }}>
                          <td style={{ padding:'7px 12px', color:'#8a93a6' }}>{i+1}</td>
                          <td style={{ padding:'7px 12px', fontWeight:600, color:'#14213d' }}>{r.student_name||'-'}</td>
                          <td style={{ padding:'7px 12px', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{r.score}/{r.max_score}</td>
                          <td style={{ padding:'7px 12px', fontWeight:700, color:scoreColor(p), fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{p}%</td>
                        </tr>
                      )})}
                    </tbody>
                  </table>
...
function TabHMDashboard({ currentUser }) {
  ...
  if (loading) return <div style={{ textAlign:'center', padding:48, color:'#5d6b82' }}>⏳ Loading HM Dashboard...</div>
  ...
  return (
    <>
      ...
      {photoView && (
        <div style={{ position:'fixed', inset:0, zIndex:99999, background:'rgba(0,0,0,.85)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }} onClick={() => setPhotoView(null)}>
          <div style={{ background:'white', borderRadius:14, padding:16, maxWidth:'94vw', maxHeight:'90vh' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize:13, fontWeight:700, color:'#14213d', marginBottom:10 }}>📸 Board Photo — {photoView.teacher} · {photoView.date}</div>
            <img src={photoView.url} alt="Board" style={{ maxWidth:'80vw', maxHeight:'70vh', borderRadius:8, objectFit:'contain' }}/>
            <div style={{ textAlign:'center', marginTop:10 }}><button onClick={() => setPhotoView(null)} style={{ padding:'8px 20px', borderRadius:8, border:'none', background:'#132a4f', color:'white', fontWeight:700, cursor:'pointer' }}>✕ Close</button></div>
          </div>
        </div>
      )}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:10, marginBottom:20 }}>
          {Object.entries(hmSummary).sort((a,b) => b[1].open - a[1].open).map(([hmName, data]) => (
            <div key={hmName}
              ...
              style={{ ...S.card, padding:14, border: selectedHouse===hmName ? '1.5px solid #132a4f' : '1px solid #e8e3d8', marginBottom:0, cursor:'pointer' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:6 }}>
                <div style={{ fontSize:14, fontWeight:800, color:'#132a4f', marginBottom:4 }}>👤 {hmName}</div>
                <a href={`/hostel?tab=doubtsession&hm=${encodeURIComponent(hmName)}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize:10, color:'#8a93a6', textDecoration:'none' }} title={`Open ${hmName}'s queue in Hostel Portal`}>🏠↗</a>
              </div>
              {data.houses.size > 0 && (
                <div style={{ fontSize:11, color:'#5d6b82', marginBottom:6 }}>
                  🏠 {[...data.houses].join(', ')}
                </div>
              )}
              <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
                ...
                {data.open===0 && data.resolved===0 && <span style={S.badge('#8a93a6','#f3f0e8')}>None</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ display:'flex', gap:10, marginBottom:16, alignItems:'center', flexWrap:'wrap' }}>
        ...
        <span style={{ fontSize:13, color:'#5d6b82' }}>{openSessions.length} open · {doneSessions.length} resolved</span>
        <button onClick={() => {
          const w = window.open('', '_blank')
          ...
          <style>
            ...
            h2{font-size:16px;margin-bottom:4px;color:#132a4f}
            p{font-size:11px;color:#555;margin-bottom:12px}
            table{width:100%;border-collapse:collapse}
            th{background:#132a4f;color:white;padding:7px 10px;text-align:left;font-size:11px}
            td{padding:6px 10px;border-bottom:1px solid #e8e3d8;font-size:12px}
          ...
          w.document.close()
        }} style={{ ...S.btnSm('#a7771f'), marginLeft:'auto' }}>🖨️ Print Summary</button>
      </div>
      <div style={S.card}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
          <h3 style={{ fontSize:15, fontWeight:800, color:'#b45309', margin:0 }}>⏳ Open Doubt Sessions ({openSessions.length})</h3>
          <a href={`/hostel?tab=doubtsession${selectedHouse !== 'All' ? `&hm=${encodeURIComponent(selectedHouse)}` : ''}`} target="_blank" rel="noopener noreferrer" style={{ fontSize:11, fontWeight:700, color:'#132a4f', textDecoration:'none', padding:'5px 10px', borderRadius:8, background:'#eef2f9', whiteSpace:'nowrap' }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {excellentLogs.filter(l => l.teaching_date === today()).map(l => (
              <div key={l.id} style={{ padding:'10px 14px', background:'white', border:'1px solid #bbf7d0', borderRadius:10, fontSize:13 }}>
                ...
                <div style={{ fontSize:11, color:'#8a93a6', marginTop:2 }}>{l.subtype || l.course}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          {(() => {
            ...
            return Object.entries(byTeacher).sort((a,b) => (b[1].blocked - a[1].blocked) || (b[1].finalWarnings - a[1].finalWarnings)).map(([name, data]) => {
              ...
              <div key={name} style={{
                ...
                >
                  <div>
                    <span style={{ fontWeight:800, fontSize:14, color:'#14213d' }}>👨‍🏫 {name}</span>
                    <div style={{ fontSize:12, color:'#5d6b82', marginTop:2 }}>{data.latest.message}</div>
                  </div>
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
                    ...
                    {data.latest.similarity_score && <span style={{ padding:'4px 10px', borderRadius:999, fontSize:11, fontWeight:600, background:'#f3f0e8', color:'#5d6b82' }}>Similarity: {data.latest.similarity_score}%</span>}
                    <span style={{ fontSize:14, color:'#8a93a6', transition:'transform .15s', transform: isExpanded ? 'rotate(180deg)' : 'none' }}>▾</span>
                  </div>
                </div>
                {isExpanded && (
                  <div style={{ marginTop:10, paddingTop:10, borderTop:'1px solid rgba(0,0,0,.06)', display:'flex', flexDirection:'column', gap:6 }}>
                    <div style={{ fontSize:11, fontWeight:700, color:'#5d6b82', textTransform:'uppercase', letterSpacing:'.05em' }}>
                    ...
                    {[...data.all].sort((a,b) => (b.created_at||'').localeCompare(a.created_at||'')).map(w => (
                      <div key={w.id} style={{
                        background:'white', border:'1px solid #f3f0e8', borderRadius:8, padding:'8px 10px',
                        display:'flex', justifyContent:'space-between', gap:10, flexWrap:'wrap',
                      }}>
                        <div style={{ flex:1, minWidth:200 }}>
                          <div style={{ fontSize:12.5, color:'#2e3b52' }}>{w.message}</div>
                          <div style={{ fontSize:11, color:'#8a93a6', marginTop:2 }}>{fmtDate(w.created_at?.split('T')[0])}</div>
                        </div>
                        <div style={{ display:'flex', gap:6, alignItems:'flex-start' }}>
                          ...
                          {w.similarity_score && <span style={{ padding:'2px 8px', borderRadius:999, fontSize:10, fontWeight:600, background:'#f3f0e8', color:'#5d6b82' }}>{w.similarity_score}%</span>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12, minWidth:500 }}>
              ...
              <tbody>
                {warnings.slice(0,20).map(w => (
                  <tr key={w.id} style={{ borderBottom:'1px solid #f3f0e8', background: w.warning_type==='blocked'?'#fff1f2':w.warning_type==='final_warning'?'#fffbeb':'white' }}>
                    <td style={{ padding:'8px 10px', color:'#8a93a6', whiteSpace:'nowrap' }}>{fmtDate(w.created_at?.split('T')[0])}</td>
                    <td style={{ padding:'8px 10px', fontWeight:600, color:'#14213d' }}>{w.teacher_name}</td>
                    ...
                    <td style={{ padding:'8px 10px', fontWeight:700, color:'#dc2626', fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>
                      {w.similarity_score ? `${w.similarity_score}%` : '—'}
                    </td>
                    <td style={{ padding:'8px 10px', color:'#5d6b82', fontSize:11 }}>{w.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <thead>
                <tr style={{ background:'#faf8f3', borderBottom:'1px solid #e8e3d8' }}>
                  {['Date','Teacher','Subject','Late','Copy-Paste','Spot-Skip','HM Unverified','Photo','Actions'].map(h => (
                    <th key={h} style={{ padding:'9px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:11, whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {flaggedLogs.map(l => (
                  <tr key={l.id} style={{ borderBottom:'1px solid #f3f0e8', background:'#fff8f8' }}>
                    <td style={{ padding:'9px 12px', color:'#5d6b82', whiteSpace:'nowrap' }}>{fmtDate(l.teaching_date)}</td>
                    <td style={{ padding:'9px 12px', fontWeight:600, color:'#14213d' }}>{l.teacher_name||'—'}</td>
                    <td style={{ padding:'9px 12px', color:'#2e3b52' }}>{l.subject_name}</td>
                    <td style={{ padding:'9px 12px' }}>{l.late_submission ? <span style={S.badge('#dc2626','#fee2e2')}>🌙 Late</span> : <span style={{ color:'#8a93a6', fontSize:12 }}>—</span>}</td>
                    <td style={{ padding:'9px 12px' }}>{l.copy_paste ? <span style={S.badge('#a7771f','#fbf3e0')}>🔁 Copy</span> : <span style={{ color:'#8a93a6', fontSize:12 }}>—</span>}</td>
                    <td style={{ padding:'9px 12px' }}>{l.spot_check_skipped ? <span style={S.badge('#d97706','#fef9c3')}>⚠️ Skipped</span> : l.spot_check_done ? <span style={S.badge('#16a34a','#dcfce7')}>✅ Done</span> : <span style={{ color:'#8a93a6', fontSize:12 }}>—</span>}</td>
                    <td style={{ padding:'9px 12px' }}>{l.hm_verified === false ? <span style={S.badge('#dc2626','#fee2e2')}>❌ Not Done</span> : l.hm_verified === true ? <span style={S.badge('#16a34a','#dcfce7')}>✅ Verified</span> : <span style={{ color:'#8a93a6', fontSize:12 }}>—</span>}</td>
      ...
      {isAdmin && teachingLogs.length > 0 && (
        <div style={S.card}>
          <h3 style={{ fontSize:15, fontWeight:800, color:'#132a4f', marginTop:0 }}>📋 All Teaching Logs (Admin)</h3>
          <div className="table-wrap">
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:600 }}>
              <thead>
                <tr style={{ background:'#faf8f3', borderBottom:'1px solid #e8e3d8' }}>
                  {['Date','Teacher','Subject','Course','Flags','Photo','Delete'].map(h => (
                    <th key={h} style={{ padding:'9px 12px', textAlign:'left', fontWeight:700, color:'#2e3b52', fontSize:11 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {teachingLogs.slice(0,50).map(l => {
                  ...
                  if (l.copy_paste)         flags.push(<span key="c" style={S.badge('#a7771f','#fbf3e0')}>🔁</span>)
                  ...
                  return (
                    <tr key={l.id} style={{ borderBottom:'1px solid #f3f0e8' }}>
                      <td style={{ padding:'9px 12px', color:'#5d6b82', whiteSpace:'nowrap' }}>{fmtDate(l.teaching_date)}</td>
                      <td style={{ padding:'9px 12px', fontWeight:600, color:'#14213d' }}>{l.teacher_name||'—'}</td>
                      <td style={{ padding:'9px 12px', color:'#2e3b52' }}>{l.subject_name}</td>
                      <td style={{ padding:'9px 12px' }}><span style={S.badge('#132a4f','#eef2f9')}>{l.subtype||l.course}</span></td>
      ...
      {isAdmin && (
        <div style={S.card}>
          <h3 style={{ fontSize:15, fontWeight:800, color:'#132a4f', marginTop:0 }}>🗑 Manage Doubt Sessions (Admin)</h3>
          <div className="table-wrap">
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:500 }}>
              ...
              <tbody>
                {allDoubt.slice(0,30).map(d => (
                  <tr key={d.id} style={{ borderBottom:'1px solid #f3f0e8' }}>
                    <td style={{ padding:'9px 12px', color:'#5d6b82', whiteSpace:'nowrap' }}>{fmtDate(d.teaching_date)}</td>
                    <td style={{ padding:'9px 12px', fontWeight:600 }}>{d.house_name||'—'}</td>
                    <td style={{ padding:'9px 12px' }}>{d.subject_name}</td>
                    <td style={{ padding:'9px 12px', color:'#5d6b82' }}>
                      {d.hm_name
                        ? <a href={`/hostel?tab=doubtsession&hm=${encodeURIComponent(d.hm_name)}`} target="_blank" rel="noopener noreferrer" style={{ color:'#132a4f', fontWeight:600, textDecoration:'underline' }} title={`Open ${d.hm_name}'s queue in Hostel Portal`}>{d.hm_name}</a>
                        : '—'}
                    </td>
                    <td style={{ padding:'9px 12px' }}>
                      {d.status==='resolved'
        ...
        <div style={S.card}>
          ...
          {weakStudents.map((w,i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:14, padding:'10px 12px', border:'1px solid #fecaca', borderRadius:8, marginBottom:6, background:'#fff1f2', flexWrap:'wrap' }}>
              <div style={{ minWidth:130, fontWeight:600, color:'#14213d', fontSize:13 }}>{w.student}</div>
              <div style={{ flex:1 }}><div style={{ fontSize:12, color:'#5d6b82', marginBottom:3 }}>{w.subject}</div><div style={{ height:5, background:'#fee2e2', borderRadius:3, overflow:'hidden' }}><div style={{ width:`${w.avg}%`, height:'100%', background:'#dc2626', borderRadius:3 }}/></div></div>
              <span style={{ fontWeight:800, color:'#dc2626', fontSize:16, fontFamily:"'Fraunces','Playfair Display',Georgia,serif" }}>{w.avg}%</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div style={S.card}>
          ...
          {histPage.map(s => (
            <div key={s.id} style={{ display:'flex', gap:10, alignItems:'center', padding:'8px 12px', border:'1px solid #bbf7d0', borderRadius:8, background:'#f0fdf4', fontSize:13, marginBottom:5, flexWrap:'wrap' }}>
              <span style={{ color:'#8a93a6', minWidth:70 }}>{fmtDate(s.teaching_date)}</span>
          ...
          {histPages > 1 && (
            <div style={{ display:'flex', gap:5, justifyContent:'center', marginTop:10 }}>
              <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1} style={{ ...S.btnSm('#5d6b82'), opacity:page===1?.4:1 }}>←</button>
              <span style={{ fontSize:13, color:'#5d6b82', padding:'6px 10px' }}>{page}/{histPages}</span>
              <button onClick={() => setPage(p => Math.min(histPages,p+1))} disabled={page===histPages} style={{ ...S.btnSm('#5d6b82'), opacity:page===histPages?.4:1 }}>→</button>
            </div>
          )}
        </div>
      )}
...
useEffect(() => {
  ...
  const badges = useMemo(() => {
    ...
    const monthLogs = logs.filter(l => l.teaching_date?.startsWith(currMonth)).length
    const batches   = new Set(timetable.map(t=>t.class_name).filter(Boolean)).size
    return { todayLogs, monthLogs, monthMissed, activeTeachers, batches }
  }, [logs, missed, timetable, todayStr, currMonth])

  const has = key => TABS.some(t => t.key === key)
  const go  = key => has(key) ? () => handleTabChange(key) : undefined
  ...
  return (
    <div className="px-root">
      ...
      <PremiumHero
        ...
        stats={[
          { label:'Logs today', value: fmt(badges.todayLogs), sub:'classes recorded', tone: badges.todayLogs ? PX.goldLt : null, onClick: go('logs'), active: activeTab==='logs' },
          { label:`Logs · ${monthName}`, value: fmt(badges.monthLogs), sub:'this month', onClick: go('calendar'), active: activeTab==='calendar' },
          { label:'Teachers active', value: fmt(badges.activeTeachers), sub:'logged this month', onClick: go('reports'), active: activeTab==='reports' },
          { label:'Missed classes', value: fmt(badges.monthMissed), sub:'this month', tone: badges.monthMissed ? '#fca5a5' : '#86efac', onClick: go('reports') },
      ...
      {TABS.length === 0 ? (
        <div className="px-card" style={{ padding:'40px 24px', textAlign:'center' }}>
          <div style={{ fontFamily:PX.serif, fontSize:20, fontWeight:600, color:PX.ink }}>No Teaching sections for your role</div>
      ...
      {has(activeTab) && activeTab==='calendar'    && <TabCalendar logs={logs} missed={missed}/>}
      {has(activeTab) && activeTab==='syllabus' && <TabSyllabus logs={logs} courseData={courseData} monthlySyllabus={monthlySyllabus} currentUser={currentUser}/>}
      {has(activeTab) && activeTab==='reports'     && <TabReports logs={logs} missed={missed} staff={staff} courseData={courseData}/>}
      {has(activeTab) && activeTab==='performance' && <TabStudentPerformance courseData={courseData} logs={logs} currentUser={currentUser}/>}
      {has(activeTab) && activeTab==='hmdash'      && <TabHMDashboard currentUser={currentUser}/>}
