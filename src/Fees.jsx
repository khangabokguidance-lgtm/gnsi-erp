...
function FeePaymentTab({ students, admissions, adm_fee_collections, adm_flat_fees, adm_course_fees, onRefresh, isAdmin, currentUser }) {
  ...
  // regardless of which session the student was actually admitted in —
  // the same bug already found and fixed in FeeCollectionModal.jsx. A
  // student admitted in 2024-2025 was being billed at whatever the
  // CURRENT session's rates were, not their own session's rates. Now
  // uses the student's own `session` field first, falling back to
  // today's session only when that's missing (e.g. legacy records
