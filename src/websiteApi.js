...
const SUB_TABS = [
  ...
  { id:"extras",     icon:"🗂️",  label:"Testimonials/Calendar/Timeline" },
...
function TestimonialsSection() {
  const [rows,setRows]=useState([]);
  const [load,setLoad]=useState(true);
  const [form,setForm]=useState({quote:"",attribution:"",rating:5,sort_order:0,is_featured:true});
  const [editing,setEdit]=useState(null);
  const [saving,setSave]=useState(false);
  ...
  const resetForm=()=>setForm({quote:"",attribution:"",rating:5,sort_order:rows.length,is_featured:true});
  ...
  const save=async()=>{
    ...
    resetForm();setEdit(null);load_();
  };
  const toggle=async(id,cur)=>{await toggleTestimonialFeatured(id,cur);toast(cur?"Unfeatured":"Featured ✓");load_();};
  const del=async id=>{if(!confirm("Delete testimonial?"))return;await deleteTestimonial(id);toast("Deleted");load_();};
  const startEdit=t=>{setEdit(t.id);setForm({quote:t.quote,attribution:t.attribution||"",rating:t.rating||5,sort_order:t.sort_order||0,is_featured:t.is_featured!==false});};
    ...
    <div>
      <div style={{...s.card,borderColor:"rgba(184,146,42,.3)",marginBottom:"1rem"}}>
        ...
        <div style={s.cardBdy}>
          <pre style={{background:"rgba(0,0,0,.3)",padding:".8rem",fontSize:".72rem",color:"#4AE382",overflowX:"auto",lineHeight:1.6,whiteSpace:"pre-wrap",marginBottom:".7rem"}}>{SQL}</pre>
          <button style={{...s.btnG,fontSize:".72rem"}} onClick={()=>{navigator.clipboard.writeText(SQL);toast("SQL copied ✓");}}>📋 Copy SQL</button>
          <p style={{color:"rgba(248,243,232,.45)",fontSize:".82rem",lineHeight:1.7,marginTop:".7rem"}}>💬 These power the "What Parents Say" quote slider. No reviewer name is shown publicly — only the <strong style={{color:C.goldL}}>attribution</strong> text you enter (e.g. "Parent of a Class VII Sainik aspirant").</p>
        </div>
      </div>
      <div style={s.card}>
        <div style={s.cardHd}><span style={s.cardTit}>{editing?"✏️ Edit Testimonial":"💬 Add Testimonial"}</span>{editing&&<button style={s.btnR} onClick={()=>{setEdit(null);resetForm();}}>Cancel</button>}</div>
        <div style={s.cardBdy}>
          <label style={s.lbl}>Quote *</label>
          <textarea style={s.ta} placeholder="e.g. GNSI transformed my son's confidence and discipline within a year." value={form.quote} onChange={e=>setForm(f=>({...f,quote:e.target.value}))}/>
          <div style={s.g3}>
            <div><label style={s.lbl}>Attribution</label><input style={s.inp} placeholder="Parent of a Class VI Navodaya aspirant" value={form.attribution} onChange={e=>setForm(f=>({...f,attribution:e.target.value}))}/></div>
          ...
          <div style={{display:"flex",gap:".8rem",alignItems:"center"}}>
            <button style={{...s.btnG,opacity:saving?.6:1}} onClick={save} disabled={saving}>{saving?"Saving…":editing?"Update Testimonial":"Add Testimonial →"}</button>
            <label style={{display:"flex",alignItems:"center",gap:".4rem",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",fontSize:".75rem",color:"rgba(248,243,232,.5)"}}>
              <input type="checkbox" checked={form.is_featured} onChange={e=>setForm(f=>({...f,is_featured:e.target.checked}))}/> Featured on website
            </label>
          </div>
        </div>
      </div>
      {load?<div style={s.loading}><Spin/>Loading…</div>:!rows.length?<div style={s.empty}>No testimonials yet — add your first parent quote</div>:rows.map(t=>(
        <div key={t.id} style={{...s.card,opacity:t.is_featured?1:.5}}>
          <div style={s.cardHd}>
            <div style={{display:"flex",alignItems:"center",gap:".7rem",flexWrap:"wrap"}}>
              {!t.is_featured&&<span style={{...s.badge("Low"),fontSize:".55rem"}}>Not Featured</span>}
              <span style={{color:C.goldL,fontFamily:"'Rajdhani',sans-serif",fontSize:".72rem"}}>{'★'.repeat(t.rating||5)}{'☆'.repeat(5-(t.rating||5))}</span>
            </div>
            <div style={{display:"flex",gap:".4rem"}}>
              <button style={s.btnG} onClick={()=>startEdit(t)}>Edit</button>
              <button style={s.btnGrn} onClick={()=>toggle(t.id,t.is_featured)}>{t.is_featured?"Unfeature":"Feature"}</button>
              <button style={s.btnR} onClick={()=>del(t.id)}>Delete</button>
            </div>
          </div>
          <div style={{padding:".6rem 1.1rem"}}>
            <p style={{color:"#F8F3E8",fontFamily:"'EB Garamond',serif",fontSize:"1rem",fontStyle:"italic",marginBottom:".3rem"}}>"{t.quote}"</p>
            <p style={{color:"rgba(248,243,232,.45)",fontSize:".78rem"}}>{t.attribution}</p>
          </div>
        </div>
      ))}
    </div>
...
function ExamCalendarSection() {
  const [rows,setRows]=useState([]);
  const [load,setLoad]=useState(true);
  const emptyForm={exam_name:"",sub_label:"",exam_type:"NVS",application_opens:"",application_closes:"",exam_date:"",status:"Upcoming",sort_order:0,exam_date_sort:""};
  ...
  sub_label           text,
  exam_type           text,       -- NVS | Sainik | RMS
  application_opens   text,
  application_closes  text,
  exam_date           text,
  exam_date_sort      date,       -- used for correct chronological ordering
  status              text default 'Upcoming', -- Upcoming | Open | Closed | Done
  sort_order          int default 0,
  created_at          timestamptz default now()
);`;
  ...
  const save=async()=>{
    ...
    setSave(false);if(error)return toast("Error: "+error.message,"error");
    toast(editing?"Row updated ✓":"Row added ✓");
    setForm({...emptyForm,sort_order:rows.length});setEdit(null);load_();
  };
  const del=async id=>{if(!confirm("Delete this exam calendar row?"))return;await deleteExamCalendarRow(id);toast("Deleted");load_();};
  const startEdit=r=>{setEdit(r.id);setForm({exam_name:r.exam_name,sub_label:r.sub_label||"",exam_type:r.exam_type||"NVS",application_opens:r.application_opens||"",application_closes:r.application_closes||"",exam_date:r.exam_date||"",status:r.status||"Upcoming",sort_order:r.sort_order||0,exam_date_sort:r.exam_date_sort||""});};

  const statusColor={Upcoming:"Med",Open:"Low",Closed:"High",Done:"Low"};
    ...
    <div>
      <div style={{...s.card,borderColor:"rgba(184,146,42,.3)",marginBottom:"1rem"}}>
        ...
        <div style={s.cardBdy}>
          <pre style={{background:"rgba(0,0,0,.3)",padding:".8rem",fontSize:".72rem",color:"#4AE382",overflowX:"auto",lineHeight:1.6,whiteSpace:"pre-wrap",marginBottom:".7rem"}}>{SQL}</pre>
          <button style={{...s.btnG,fontSize:".72rem"}} onClick={()=>{navigator.clipboard.writeText(SQL);toast("SQL copied ✓");}}>📋 Copy SQL</button>
          <p style={{color:"rgba(248,243,232,.45)",fontSize:".82rem",lineHeight:1.7,marginTop:".7rem"}}>📅 This drives the exam comparison table on the landing page. Set <strong style={{color:C.goldL}}>exam_date_sort</strong> (a real date) so rows sort correctly even though the displayed date text can be freeform (e.g. "Second Sunday of Dec").</p>
        </div>
      </div>
      <div style={s.card}>
        <div style={s.cardHd}><span style={s.cardTit}>{editing?"✏️ Edit Row":"📅 Add Exam Calendar Row"}</span>{editing&&<button style={s.btnR} onClick={()=>{setEdit(null);setForm(emptyForm);}}>Cancel</button>}</div>
        <div style={s.cardBdy}>
          <div style={s.g2}>
            <div><label style={s.lbl}>Exam Name *</label><input style={s.inp} placeholder="e.g. JNVST Class VI" value={form.exam_name} onChange={e=>setForm(f=>({...f,exam_name:e.target.value}))}/></div>
            <div><label style={s.lbl}>Sub-label</label><input style={s.inp} placeholder="e.g. Navodaya Vidyalaya Samiti" value={form.sub_label} onChange={e=>setForm(f=>({...f,sub_label:e.target.value}))}/></div>
          </div>
          <div style={s.g3}>
            <div>
              <label style={s.lbl}>Exam Type</label>
              <select style={s.sel} value={form.exam_type} onChange={e=>setForm(f=>({...f,exam_type:e.target.value}))}>
                <option value="NVS">NVS</option><option value="Sainik">Sainik</option><option value="RMS">RMS</option>
              </select>
            </div>
            <div>
              <label style={s.lbl}>Status</label>
              <select style={s.sel} value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))}>
                <option value="Upcoming">Upcoming</option><option value="Open">Open</option><option value="Closed">Closed</option><option value="Done">Done</option>
              </select>
            </div>
            <div><label style={s.lbl}>Sort Order</label><input style={s.inp} type="number" value={form.sort_order} onChange={e=>setForm(f=>({...f,sort_order:Number(e.target.value)}))}/></div>
          </div>
          <div style={s.g3}>
            <div><label style={s.lbl}>Application Opens</label><input style={s.inp} placeholder="e.g. 1 July 2026" value={form.application_opens} onChange={e=>setForm(f=>({...f,application_opens:e.target.value}))}/></div>
            <div><label style={s.lbl}>Application Closes</label><input style={s.inp} placeholder="e.g. 15 Sept 2026" value={form.application_closes} onChange={e=>setForm(f=>({...f,application_closes:e.target.value}))}/></div>
            <div><label style={s.lbl}>Exam Date (display text)</label><input style={s.inp} placeholder="e.g. 13 Dec 2026" value={form.exam_date} onChange={e=>setForm(f=>({...f,exam_date:e.target.value}))}/></div>
          </div>
          <label style={s.lbl}>Exam Date (for sorting)</label>
          <input style={s.inp} type="date" value={form.exam_date_sort} onChange={e=>setForm(f=>({...f,exam_date_sort:e.target.value}))}/>
          <button style={{...s.btnG,opacity:saving?.6:1}} onClick={save} disabled={saving}>{saving?"Saving…":editing?"Update Row":"Add Row →"}</button>
        </div>
      </div>
      {load?<div style={s.loading}><Spin/>Loading…</div>:!rows.length?<div style={s.empty}>No exam calendar rows yet</div>:rows.map(r=>(
        <div key={r.id} style={s.card}>
          <div style={s.cardHd}>
            <div style={{display:"flex",alignItems:"center",gap:".7rem",flexWrap:"wrap"}}>
              <span style={s.badge(statusColor[r.status]||"Med")}>{r.status}</span>
              <span style={{color:C.goldL,fontFamily:"'Rajdhani',sans-serif",fontSize:".72rem"}}>{r.exam_type}</span>
              <span style={{color:"#F8F3E8",fontFamily:"'EB Garamond',serif",fontSize:"1rem"}}>{r.exam_name}</span>
            </div>
            <div style={{display:"flex",gap:".4rem"}}>
              <button style={s.btnG} onClick={()=>startEdit(r)}>Edit</button>
              <button style={s.btnR} onClick={()=>del(r.id)}>Delete</button>
          ...
          <div style={{padding:".6rem 1.1rem",display:"flex",gap:"1.4rem",flexWrap:"wrap"}}>
            <span style={{color:"rgba(248,243,232,.45)",fontSize:".78rem"}}>Opens: {r.application_opens||"—"}</span>
            <span style={{color:"rgba(248,243,232,.45)",fontSize:".78rem"}}>Closes: {r.application_closes||"—"}</span>
            <span style={{color:"rgba(248,243,232,.45)",fontSize:".78rem"}}>Exam: {r.exam_date||"—"}</span>
          </div>
        </div>
      ))}
    </div>
...
function TimelineSection() {
  const [rows,setRows]=useState([]);
  const [load,setLoad]=useState(true);
  const emptyForm={title:"",description:"",event_date:"",month_label:"",day_label:"",status:"upcoming",tag_html:"",sort_order:0};
  const [form,setForm]=useState(emptyForm);
  const [editing,setEdit]=useState(null);
  const [saving,setSave]=useState(false);
  ...
  title        text not null,
  description  text,
  event_date   date,        -- preferred: drives month/day display automatically
  month_label  text,        -- fallback if event_date not set, e.g. 'Dec'
  day_label    text,        -- fallback if event_date not set, e.g. '13'
  status       text default 'upcoming', -- done | open | upcoming
  tag_html     text,        -- optional small HTML tag/badge shown on the card
  sort_order   int default 0,
  created_at   timestamptz default now()
);`;
  ...
  const startEdit=t=>{setEdit(t.id);setForm({title:t.title,description:t.description||"",event_date:t.event_date||"",month_label:t.month_label||"",day_label:t.day_label||"",status:t.status||"upcoming",tag_html:t.tag_html||"",sort_order:t.sort_order||0});};

  const statusColor={upcoming:"Med",open:"Low",done:"Low"};
    ...
    <div>
      <div style={{...s.card,borderColor:"rgba(184,146,42,.3)",marginBottom:"1rem"}}>
        ...
        <div style={s.cardBdy}>
          <pre style={{background:"rgba(0,0,0,.3)",padding:".8rem",fontSize:".72rem",color:"#4AE382",overflowX:"auto",lineHeight:1.6,whiteSpace:"pre-wrap",marginBottom:".7rem"}}>{SQL}</pre>
          <button style={{...s.btnG,fontSize:".72rem"}} onClick={()=>{navigator.clipboard.writeText(SQL);toast("SQL copied ✓");}}>📋 Copy SQL</button>
          <p style={{color:"rgba(248,243,232,.45)",fontSize:".82rem",lineHeight:1.7,marginTop:".7rem"}}>🗓️ Powers the "Important Dates" timeline strip. If you set <strong style={{color:C.goldL}}>Event Date</strong>, month/day are derived automatically — the manual Month/Day labels are only used as a fallback.</p>
        </div>
      </div>
      <div style={s.card}>
        <div style={s.cardHd}><span style={s.cardTit}>{editing?"✏️ Edit Timeline Item":"🗓️ Add Timeline Item"}</span>{editing&&<button style={s.btnR} onClick={()=>{setEdit(null);setForm(emptyForm);}}>Cancel</button>}</div>
        <div style={s.cardBdy}>
          <label style={s.lbl}>Title *</label>
          <input style={s.inp} placeholder="e.g. Admit Cards Released" value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}/>
          <label style={s.lbl}>Description</label>
          <textarea style={s.ta} placeholder="Short supporting detail shown under the title" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/>
          <div style={s.g3}>
            <div><label style={s.lbl}>Event Date (preferred)</label><input style={s.inp} type="date" value={form.event_date} onChange={e=>setForm(f=>({...f,event_date:e.target.value}))}/></div>
            <div><label style={s.lbl}>Month Label (fallback)</label><input style={s.inp} placeholder="Dec" value={form.month_label} onChange={e=>setForm(f=>({...f,month_label:e.target.value}))}/></div>
            <div><label style={s.lbl}>Day Label (fallback)</label><input style={s.inp} placeholder="13" value={form.day_label} onChange={e=>setForm(f=>({...f,day_label:e.target.value}))}/></div>
          </div>
          <div style={s.g3}>
            <div>
              <label style={s.lbl}>Status</label>
              <select style={s.sel} value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))}>
                <option value="upcoming">Upcoming</option><option value="open">Open</option><option value="done">Done</option>
              </select>
            </div>
            <div><label style={s.lbl}>Tag HTML (optional)</label><input style={s.inp} placeholder="&lt;span&gt;New&lt;/span&gt;" value={form.tag_html} onChange={e=>setForm(f=>({...f,tag_html:e.target.value}))}/></div>
            <div><label style={s.lbl}>Sort Order</label><input style={s.inp} type="number" value={form.sort_order} onChange={e=>setForm(f=>({...f,sort_order:Number(e.target.value)}))}/></div>
          </div>
          <button style={{...s.btnG,opacity:saving?.6:1}} onClick={save} disabled={saving}>{saving?"Saving…":editing?"Update Item":"Add Item →"}</button>
        </div>
      </div>
      {load?<div style={s.loading}><Spin/>Loading…</div>:!rows.length?<div style={s.empty}>No timeline items yet</div>:rows.map(t=>(
        <div key={t.id} style={s.card}>
          <div style={s.cardHd}>
            <div style={{display:"flex",alignItems:"center",gap:".7rem",flexWrap:"wrap"}}>
              <span style={s.badge(statusColor[t.status]||"Med")}>{t.status}</span>
              <span style={{color:C.goldL,fontFamily:"'Rajdhani',sans-serif",fontSize:".72rem"}}>{t.event_date||`${t.month_label||''} ${t.day_label||''}`}</span>
              <span style={{color:"#F8F3E8",fontFamily:"'EB Garamond',serif",fontSize:"1rem"}}>{t.title}</span>
            </div>
            <div style={{display:"flex",gap:".4rem"}}>
              <button style={s.btnG} onClick={()=>startEdit(t)}>Edit</button>
              <button style={s.btnR} onClick={()=>del(t.id)}>Delete</button>
          ...
          {t.description&&<div style={{padding:".6rem 1.1rem"}}><p style={{color:"rgba(248,243,232,.45)",fontSize:".82rem"}}>{t.description}</p></div>}
        </div>
      ))}
    </div>
  );
...
// ════════════════════════════════════════════════════════════
//  EXTRAS — groups Testimonials / Exam Calendar / Timeline
// ════════════════════════════════════════════════════════════
function ExtrasSection() {
  const [inner,setInner]=useState("testimonials");
  const INNER_TABS=[
    {id:"testimonials",icon:"💬",label:"Testimonials"},
    {id:"examcal",icon:"📅",label:"Exam Calendar"},
    {id:"timeline",icon:"🗓️",label:"Important Dates"},
  ];
  const PANELS={
  ...
  return (
    <div>
      <div style={{...s.subNav,marginBottom:"1.2rem"}}>
        {INNER_TABS.map(t=>(
          <button key={t.id} style={s.subBtn(inner===t.id)} onClick={()=>setInner(t.id)}>
