...
//  Bulk admission fee collection for migrated students
//  Writes identical records to:
//    - adm_fee_collections  (same schema as FeeCollectionModal)
//    - accounts             (same schema as FeeCollectionModal)
//  Generates per-student receipts (printable)
// ============================================================
...
// ─── Constants (must match FeeCollectionModal exactly) ────────
...
const rcptNo  = () => 'GNSI-' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).slice(2,5).toUpperCase()
const gccStr  = gcc => String(parseInt(gcc) || gcc) // strips .0 float issue

const getDefaultFee = (course, hostelType) => {
  const ck = Object.keys(COURSE_FEES).find(k => (course||'').toLowerCase().includes(k.toLowerCase())) || course
  const hk = hostelType === 'Hostel' ? 'Boarder' : (hostelType || 'Day Scholar')
  return COURSE_FEES[ck]?.[hk] || COURSE_FEES[ck]?.['Day Scholar'] || DEFAULT_ADM_FEE
}
...
function ReceiptPrinter({ receipts, onClose }) {
  ...
  const print = () => {
    ...
    w.document.write(`
      <html><head><title>GNSI Admission Fee Receipts</title>
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body { font-family: 'Georgia', serif; background: #f5f5f0; }
        .page { padding: 20px; }
        .receipt {
          width: 100%; max-width: 680px; margin: 0 auto 32px;
          border: 2px solid #0f2744; border-radius: 4px;
        ...
        .inst-name { font-size: 18px; font-weight: 700; letter-spacing: .5px; }
        .inst-sub  { font-size: 10px; color: #c9a84c; margin-top: 3px; letter-spacing: 1px; text-transform: uppercase; }
        .rcpt-label { text-align: right; }
        .rcpt-no   { font-size: 13px; font-weight: 700; color: #c9a84c; }
        .rcpt-date { font-size: 10px; color: #94a3b8; margin-top: 2px; }
        .receipt-body { padding: 18px 24px; }
        .student-row { display: flex; gap: 24px; margin-bottom: 16px; padding-bottom: 14px; border-bottom: 1px solid #e2e8f0; }
        .field { flex: 1; }
        .field-label { font-size: 9px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: .08em; margin-bottom: 3px; }
        .field-value { font-size: 13px; font-weight: 600; color: #0f172a; }
        .gcc { color: #0f2744; font-weight: 800; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 14px; }
        th { background: #f8fafc; padding: 7px 10px; font-size: 10px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: .06em; text-align: left; border-bottom: 1px solid #e2e8f0; }
        td { padding: 8px 10px; font-size: 12px; color: #334155; border-bottom: 1px solid #f1f5f9; }
        .amount-col { text-align: right; font-weight: 700; color: #0f2744; }
        .total-row td { background: #0f2744; color: white; font-weight: 700; font-size: 13px; }
        .total-row .amount-col { color: #c9a84c; font-size: 15px; }
        .pay-row { display: flex; gap: 16px; margin-bottom: 14px; }
        .pay-badge { padding: 4px 12px; border-radius: 99px; background: #ecfdf5; color: #059669; font-size: 11px; font-weight: 700; border: 1px solid #6ee7b7; }
        .footer { display: flex; justify-content: space-between; align-items: flex-end; padding-top: 14px; border-top: 1px dashed #e2e8f0; }
        .sig-line { border-top: 1px solid #334155; width: 140px; padding-top: 4px; font-size: 10px; color: #64748b; text-align: center; }
        .watermark { text-align: center; font-size: 10px; color: #94a3b8; margin-top: 10px; }
        @media print {
          body { background: white; }
          .receipt { border: 1px solid #ccc; margin-bottom: 0; }
        }
      ...
      ${receipts.map(r => `
        <div class="receipt">
          <div class="receipt-header">
            <div>
              <div class="inst-name">🎓 GNSI</div>
              <div class="inst-sub">Guidance Navodaya & Sainik Institute</div>
              <div style="font-size:10px;color:#94a3b8;margin-top:4px">Khangabok, Thoubal, Manipur</div>
            </div>
            <div class="rcpt-label">
              <div style="font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.08em">Admission Fee Receipt</div>
              <div class="rcpt-no">${r.receipt_no}</div>
              <div class="rcpt-date">${new Date(r.pay_date).toLocaleDateString('en-IN', { day:'2-digit', month:'short', year:'numeric' })}</div>
            </div>
          </div>
          <div class="receipt-body">
            <div class="student-row">
              <div class="field">
                ...
                <div class="field-value">${r.student_name}</div>
              </div>
              <div class="field">
                <div class="field-label">GCC No.</div>
                <div class="field-value gcc">GCC-${r.gcc}</div>
              </div>
              <div class="field">
                <div class="field-label">Course</div>
                <div class="field-value">${r.course || '—'}</div>
              </div>
              <div class="field">
                <div class="field-label">Class</div>
                <div class="field-value">${r.batch || '—'}</div>
              </div>
            </div>
            <table>
              <thead>
              ...
              <tbody>
                ${r.items.map(it => `
                  <tr>
                    <td>${it.label}</td>
                    <td class="amount-col">₹${fmt(it.amount)}</td>
                  </tr>
                `).join('')}
                <tr class="total-row">
            ...
            <div class="pay-row">
              <span class="pay-badge">✓ ${r.pay_mode}</span>
              ${r.txn_ref ? `<span style="font-size:11px;color:#64748b">Ref: ${r.txn_ref}</span>` : ''}
              ${r.collected_by ? `<span style="font-size:11px;color:#64748b">Collected by: ${r.collected_by}</span>` : ''}
            </div>
            <div class="footer">
              <div>
              ...
              <div style="text-align:right">
                <div style="font-size:10px;color:#94a3b8">Amount in words:</div>
                <div style="font-size:11px;font-weight:600;color:#334155">${amountInWords(r.total)}</div>
              </div>
            </div>
            <div class="watermark">This is a computer-generated receipt — GNSI Portal</div>
          </div>
        </div>
      `).join('')}
      </div></body></html>
        ...
        <div style={{ maxHeight:220, overflowY:'auto', marginBottom:20, display:'flex', flexDirection:'column', gap:8 }}>
          {receipts.map(r => (
            <div key={r.receipt_no} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', background:C.slate[50], borderRadius:10, border:`1px solid ${C.slate[200]}` }}>
              <div style={{ width:36, height:36, borderRadius:10, background:C.navy, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:12, fontWeight:800, flexShrink:0 }}>
...
export default function BulkAdmissionFee() {
  // ── Data state ───────────────────────────────────────────────
  const [students,    setStudents]    = useState([])   // all enrolled with no adm fee
  const [loading,     setLoading]     = useState(true)
  const [error,       setError]       = useState(null)

  // ── Selection + fee editing ──────────────────────────────────
  const [selected,    setSelected]    = useState({})   // { gcc: true/false }
  const [feeEdits,    setFeeEdits]    = useState({})   // { gcc: { admission, dress, prospectus } }
  const [feeItems,    setFeeItems]    = useState({})   // { gcc: { admission: true, dress: false, ... } }
  ...
  const [receipts,    setReceipts]    = useState([])   // for print
  ...
  // ── Load enrolled students with no admission fee ─────────────
  const load = useCallback(async () => {
    setLoading(true)
    setError(null)
    try {
      // Get all enrolled students from admissions table
      ...
      // Get all existing admission fee records
      ...
      const paidSet = new Set((paid || []).map(p => String(parseInt(p.adm_app_id))))

      // Filter to only those without admission fee
      const unpaid = (apps || []).filter(a => !paidSet.has(gccStr(a.gcc_no)))

      setStudents(unpaid)

      // Init fee edits with defaults
      const edits = {}
      const items = {}
      unpaid.forEach(s => {
        const gc = gccStr(s.gcc_no)
        edits[gc] = {
          admission:  DEFAULT_ADM_FEE,
          dress:      DEFAULT_DRESS,
          prospectus: DEFAULT_PROSP,
        }
  ...
  // ── Set global fee for all selected ─────────────────────────
  const applyGlobalFee = (item, val) => {
    const selectedKeys = Object.keys(selected).filter(k => selected[k])
    setFeeEdits(p => {
      const next = { ...p }
      selectedKeys.forEach(gc => { next[gc] = { ...next[gc], [item]: Number(val) || 0 } })
      return next
    })
  }
  ...
  const totalAmount  = selectedList.reduce((sum, s) => {
    ...
    return sum + (its.admission ? (amts.admission || DEFAULT_ADM_FEE) : 0)
  ...
  const saveAll = async () => {
    ...
    if (!window.confirm(`Record admission fees for ${selectedList.length} students? Total: ₹${fmt(totalAmount)}`)) return
    ...
    for (let i = 0; i < selectedList.length; i++) {
      ...
      const lineItems = [
        its.admission  && { id:'admission',  label:'Admission Fee',  type:'admission', amount: amts.admission  || DEFAULT_ADM_FEE, icon:'🎓' },
        its.dress      && { id:'dress',      label:'Dress Fee',       type:'item',      amount: amts.dress      || DEFAULT_DRESS,   icon:'👕' },
        its.prospectus && { id:'prospectus', label:'Prospectus Fee',  type:'item',      amount: amts.prospectus || DEFAULT_PROSP,   icon:'📖' },
      ].filter(Boolean)
      ...
      try {
        // Insert each fee line into adm_fee_collections
        ...
        // Insert into accounts (identical to FeeCollectionModal)
      ...
      // Small delay to avoid rate limiting on large batches
      if (i % 10 === 9) await new Promise(r => setTimeout(r, 200))
    }

    setSaving(false)
    setSavedCount(newReceipts.length)
    ...
    if (newReceipts.length) {
      setShowPrint(true)
      // Remove saved students from list
  ...
  // ─── Render ───────────────────────────────────────────────────
  return (
    <div style={{ minHeight:'100vh', background:'#f0f4f8', fontFamily:"'DM Sans', system-ui, sans-serif", padding:'0 0 60px' }}>
        ...
        {loading ? (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {filtered.map(s => {
              ...
              return (
                <div key={gc} style={{
                  background:'white', borderRadius:12, border:`1.5px solid ${sel ? C.navy : C.slate[200]}`,
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:14, padding:'13px 18px', cursor:'pointer' }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:11, color:C.slate[400], display:'flex', gap:8, flexWrap:'wrap', marginTop:2 }}>
                        ...
                        {s.hostel_type && <span style={{ color: s.hostel_type==='Boarder'?'#1d4ed8':s.hostel_type==='Day Boarder'?'#d97706':'#059669', fontWeight:600 }}>{s.hostel_type}</span>}
                      </div>
                    </div>
                  ...
                  {/* Fee item selector (always visible) */}
          ...
          <div style={{
            ...
            {saving ? (
              ...
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button onClick={saveAll} style={{ padding:'10px 28px', borderRadius:9, border:'none', background:C.gold, color:C.navy, fontSize:14, fontWeight:800, cursor:'pointer', letterSpacing:'-.01em' }}>
