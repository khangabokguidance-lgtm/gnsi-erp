...
//  3-level fee hierarchy: Course → Academic Subtype → Hostel Type
//  Tables: courses, course_batches, course_enrollments, course_fees
//  + students table for GCC lookup
// ============================================================
...
const COURSE_STRUCTURE = {
  Navodaya:          { subtypes: ["Lakshya", "Umeed"],               color: "#1D4ED8", bg: "#EFF6FF", border: "#BFDBFE" },
  Sainik:            { subtypes: ["Achiever", "Leader", "Champion"], color: "#16A34A", bg: "#F0FDF4", border: "#BBF7D0" },
  Foundation:        { subtypes: ["Elite", "Prime"],                 color: "#7C3AED", bg: "#F5F3FF", border: "#DDD6FE" },
  "Combined Course": { subtypes: [],                                 color: "#EA580C", bg: "#FFF7ED", border: "#FED7AA" },
};
...
const NAV_TABS = [
  ...
  { id: "courses",     icon: "📚", label: "Courses & Subtypes" },
...
function CourseBadge({ course }) {
  const c = COURSE_STRUCTURE[course] || { color: "#374151", bg: "#F3F4F6", border: "#E5E7EB" };
  ...
  useEffect(() => {
    (async () => {
      const [
        ...
        supabase.from("course_enrollments").select("id,student_name,gcc_no,course,subtype,hostel_type,class_name,enrolled_at").order("enrolled_at", { ascending: false }).limit(6),
      ]);
      setStats({ totalCourses, totalBatches, totalEnrollments, activeEnrollments });
      setRecent(rec || []);

      const cc = await Promise.all(Object.keys(COURSE_STRUCTURE).map(async course => {
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
        <StatCard label="Total Courses"     value={stats.totalCourses ?? 0}     icon="📚" color="#1D4ED8" bg="#EFF6FF" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        <div style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: 12, padding: 22 }}>
          ...
          {counts.map(({ course, count }) => {
            const c   = COURSE_STRUCTURE[course];
...
function CoursesSection() {
  ...
  const [form, setForm] = useState({ course: "Navodaya", subtype: "", hostel_type: "", class_name: "", description: "", duration_months: "", max_seats: "" });

  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from("courses").select("*").order("course").order("subtype");
    setCourses(data || []);
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);

  const openAdd  = () => { setEditing(null); setForm({ course: "Navodaya", subtype: "", hostel_type: "", class_name: "", description: "", duration_months: "", max_seats: "" }); setErr(null); setShowModal(true); };
  const openEdit = c  => { setEditing(c); setForm({ course: c.course, subtype: c.subtype ?? "", hostel_type: c.hostel_type ?? "", class_name: c.class_name ?? "", description: c.description ?? "", duration_months: c.duration_months ?? "", max_seats: c.max_seats ?? "" }); setErr(null); setShowModal(true); };
  const del      = async id => { if (!window.confirm("Delete?")) return; await supabase.from("courses").delete().eq("id", id); load(); };

  const save = async () => {
    if (!form.course) { setErr("Course required."); return; }
    setSaving(true); setErr(null);
    const payload = { course: form.course, subtype: form.subtype || null, hostel_type: form.hostel_type || null, class_name: form.class_name || null, description: form.description || null, duration_months: form.duration_months ? parseInt(form.duration_months) : null, max_seats: form.max_seats ? parseInt(form.max_seats) : null };
    const { error } = editing ? await supabase.from("courses").update(payload).eq("id", editing.id) : await supabase.from("courses").insert(payload);
  ...
  const grouped  = {};
  courses.forEach(c => { if (!grouped[c.course]) grouped[c.course] = []; grouped[c.course].push(c); });
  ...
  return (
    <div>
      {showModal && (
        <Modal title={editing ? "Edit Course Entry" : "Add Course Entry"} onClose={() => setShowModal(false)}>
          {err && <ErrBox msg={err} />}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              <label style={S.lbl}>Academic Subtype</label>
              {subtypes.length > 0
                ? <select value={form.subtype} onChange={e => setForm(f => ({ ...f, subtype: e.target.value }))} style={S.inp()}><option value="">— All —</option>{subtypes.map(s => <option key={s}>{s}</option>)}</select>
            ...
            <div>
              <label style={S.lbl}>Duration (months)</label>
              <input type="number" value={form.duration_months} onChange={e => setForm(f => ({ ...f, duration_months: e.target.value }))} placeholder="12" style={S.inp()} />
            </div>
            <div>
              <label style={S.lbl}>Max Seats</label>
              <input type="number" value={form.max_seats} onChange={e => setForm(f => ({ ...f, max_seats: e.target.value }))} placeholder="30" style={S.inp()} />
            </div>
            <div style={{ gridColumn: "1/-1" }}>
              <label style={S.lbl}>Description</label>
              <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} style={{ ...S.inp(), resize: "vertical" }} />
            </div>
          </div>
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
            <button onClick={() => setShowModal(false)} style={S.btn("#F3F4F6", "#374151")}>Cancel</button>
            <button onClick={save} disabled={saving} style={S.btn(saving ? "#93C5FD" : "#1D4ED8")}>{saving ? "Saving…" : editing ? "Update" : "Add"}</button>
          </div>
        </Modal>
      )}

      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 20 }}>
        <button onClick={openAdd} style={S.btn()}>+ Add Course Entry</button>
      </div>

      {Object.keys(COURSE_STRUCTURE).map(courseName => {
        const entries = grouped[courseName] || [];
        const c = COURSE_STRUCTURE[courseName];
        return (
          <div key={courseName} style={{ marginBottom: 28 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <span style={{ fontWeight: 700, fontSize: 15, color: c.color }}>{courseName}</span>
              <span style={{ fontSize: 12, color: "#9CA3AF" }}>{entries.length} entries</span>
            </div>
            {entries.length === 0
              ? <div style={{ padding: "14px 18px", background: "#F9FAFB", borderRadius: 10, fontSize: 13, color: "#9CA3AF" }}>No entries yet.</div>
              : (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 12 }}>
                  {entries.map(entry => (
                    <div key={entry.id} style={{ padding: 16, border: `1px solid ${c.border}`, borderRadius: 10, background: c.bg }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13, color: c.color }}>{entry.subtype || "All Subtypes"}</div>
                          <div style={{ fontSize: 11, color: "#6B7280", marginTop: 2 }}>{entry.class_name || "All classes"}</div>
                        </div>
                        <div style={{ display: "flex", gap: 5 }}>
                          <button onClick={() => openEdit(entry)} style={S.btn("white", c.color, { border: `1px solid ${c.border}`, padding: "3px 8px", fontSize: 11 })}>Edit</button>
                          <button onClick={() => del(entry.id)}   style={S.btn("white", "#DC2626", { border: "1px solid #FECACA", padding: "3px 8px", fontSize: 11 })}>Del</button>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {entry.hostel_type && <HostelBadge type={entry.hostel_type} />}
                        {entry.duration_months && <span style={{ fontSize: 11, padding: "2px 8px", background: "white", borderRadius: 4, border: `1px solid ${c.border}`, color: c.color }}>⏱ {entry.duration_months}m</span>}
                        {entry.max_seats      && <span style={{ fontSize: 11, padding: "2px 8px", background: "white", borderRadius: 4, border: `1px solid ${c.border}`, color: c.color }}>💺 {entry.max_seats}</span>}
                      </div>
                    </div>
                  ))}
                </div>
...
function BatchesSection() {
  ...
  const emptyForm = { batch_name: "", course: "Navodaya", subtype: "", hostel_type: "", class_name: "", session_year: "2025-2026", start_time: "", end_time: "", days: [], start_date: "", end_date: "", teacher_name: "", room: "", capacity: "", status: "Active" };
  const [form, setForm] = useState(emptyForm);

  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from("course_batches").select("*").order("session_year", { ascending: false }).order("created_at", { ascending: false });
    setBatches(data || []);
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const del      = async id => { if (!window.confirm("Delete batch?")) return; await supabase.from("course_batches").delete().eq("id", id); load(); };
  ...
  const save = async () => {
    ...
    setSaving(true); setErr(null);
    const payload = { batch_name: form.batch_name, course: form.course, subtype: form.subtype || null, hostel_type: form.hostel_type || null, class_name: form.class_name || null, session_year: form.session_year || null, start_time: form.start_time || null, end_time: form.end_time || null, days: form.days, start_date: form.start_date || null, end_date: form.end_date || null, teacher_name: form.teacher_name || null, room: form.room || null, capacity: form.capacity ? parseInt(form.capacity) : null, status: form.status };
    const { error } = editing ? await supabase.from("course_batches").update(payload).eq("id", editing.id) : await supabase.from("course_batches").insert(payload);
    if (error) { setErr(error.message); setSaving(false); return; }
    setSaving(false); setShowModal(false); load();
  };

  const sessions = ["All", ...new Set(batches.map(b => b.session_year).filter(Boolean).sort().reverse())];
  const filtered = sessionFilter === "All" ? batches : batches.filter(b => b.session_year === sessionFilter);
  const subtypes = COURSE_STRUCTURE[form.course]?.subtypes ?? [];
  ...
  return (
    <div>
        ...
        <Modal title={editing ? "Edit Batch" : "Add Batch"} onClose={() => setShowModal(false)} wide>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ gridColumn: "1/-1" }}>
              ...
              <input value={form.batch_name} onChange={e => setForm(f => ({ ...f, batch_name: e.target.value }))} placeholder="e.g. Sainik Achiever Boarder 2025-26 Morning" style={S.inp()} />
            </div>
            <div>
              <label style={S.lbl}>Course *</label>
              <select value={form.course} onChange={e => setForm(f => ({ ...f, course: e.target.value, subtype: "" }))} style={S.inp()}>
            ...
            <div>
              <label style={S.lbl}>Class Name</label>
              <input value={form.class_name} onChange={e => setForm(f => ({ ...f, class_name: e.target.value }))} placeholder="Class 6" style={S.inp()} />
            </div>
            <div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {sessions.map(s => (
            <button key={s} onClick={() => setSessionFilter(s)}
              style={{ padding: "6px 14px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "inherit", border: sessionFilter === s ? "1.5px solid #1D4ED8" : "1px solid #E5E7EB", background: sessionFilter === s ? "#EFF6FF" : "white", color: sessionFilter === s ? "#1D4ED8" : "#374151", fontWeight: sessionFilter === s ? 600 : 400 }}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 16 }}>
        {filtered.map(b => {
          const c = COURSE_STRUCTURE[b.course] || { color: "#374151", bg: "#F9FAFB", border: "#E5E7EB" };
          return (
...
// ─────────────────────────────────────────────────────────────
// 4. ENROLLMENTS — GCC lookup + hostel auto-fill
// ─────────────────────────────────────────────────────────────
function EnrollmentsSection() {
  ...
  const emptyForm = { student_name: "", student_id: "", gcc_no: "", hostel_type: "", course: "Navodaya", subtype: "", class_name: "", session_year: "2025-2026", batch_id: "", enrolled_at: new Date().toISOString().slice(0, 10), status: "Active", notes: "" };
  const [form, setForm] = useState(emptyForm);

  const load = useCallback(async () => {
    setLoading(true);
    const [{ data: enr }, { data: bat }, { data: stu }] = await Promise.all([
      supabase.from("course_enrollments").select("*").order("enrolled_at", { ascending: false }),
      supabase.from("course_batches").select("id,batch_name,course,subtype,hostel_type,session_year"),
  ...
  const save = async () => {
    ...
    setSaving(true); setErr(null);
    const payload = { student_name: form.student_name, student_id: form.student_id || null, gcc_no: form.gcc_no || null, hostel_type: form.hostel_type || null, course: form.course, subtype: form.subtype || null, class_name: form.class_name || null, session_year: form.session_year || null, batch_id: form.batch_id || null, enrolled_at: form.enrolled_at || null, status: form.status, notes: form.notes || null };
    const { error } = editing ? await supabase.from("course_enrollments").update(payload).eq("id", editing.id) : await supabase.from("course_enrollments").insert(payload);
    if (error) { setErr(error.message); setSaving(false); return; }
    setSaving(false); setShowModal(false); load();
  };
  ...
  return (
    <div>
        ...
        <Modal title={editing ? "Edit Enrollment" : "Enroll Student"} onClose={() => setShowModal(false)} wide>
          ...
          {form.student_id && (
            <div style={{ marginBottom: 14, padding: "10px 14px", background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 8, display: "flex", alignItems: "center", gap: 10, fontSize: 13, flexWrap: "wrap" }}>
              ...
              <button onClick={() => setForm(f => ({ ...f, student_id: "", gcc_no: "", student_name: "", hostel_type: "" }))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              <label style={S.lbl}>Course *</label>
              <select value={form.course} onChange={e => setForm(f => ({ ...f, course: e.target.value, subtype: "", batch_id: "" }))} style={S.inp()}>
                {Object.keys(COURSE_STRUCTURE).map(c => <option key={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label style={S.lbl}>Academic Subtype</label>
              {subtypes.length > 0
                ? <select value={form.subtype} onChange={e => setForm(f => ({ ...f, subtype: e.target.value }))} style={S.inp()}><option value="">— Select —</option>{subtypes.map(s => <option key={s}>{s}</option>)}</select>
                : <input value={form.subtype} onChange={e => setForm(f => ({ ...f, subtype: e.target.value }))} style={S.inp()} />
              }
            </div>
            <div>
            ...
            <div>
              <label style={S.lbl}>Class Name</label>
              <input value={form.class_name} onChange={e => setForm(f => ({ ...f, class_name: e.target.value }))} style={S.inp()} />
            </div>
            <div>
              <label style={S.lbl}>Session Year</label>
              <input value={form.session_year} onChange={e => setForm(f => ({ ...f, session_year: e.target.value }))} placeholder="2025-2026" style={S.inp()} />
            ...
            <div>
              <label style={S.lbl}>Batch {form.hostel_type ? `(filtered to ${form.hostel_type})` : ""}</label>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        {["All", ...Object.keys(COURSE_STRUCTURE)].map(f => (
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: "#F9FAFB", borderBottom: "1px solid #E5E7EB" }}>
              {["Student","GCC","Course","Subtype","Hostel","Class","Session","Batch","Status","Actions"].map(h => (
          ...
          <tbody>
            {filtered.map((e, i) => (
              ...
              >
                ...
                <td style={{ padding: "11px 14px", color: "#6B7280", fontSize: 12 }}>{batches.find(b => b.id === e.batch_id)?.batch_name || "—"}</td>
...
// ─────────────────────────────────────────────────────────────
// 5. FEE STRUCTURE — Course × Subtype × Hostel Type (3-level)
// ─────────────────────────────────────────────────────────────
function FeesSection() {
  ...
  const emptyForm = { course: "Navodaya", subtype: "", hostel_type: "", class_name: "", fee_type: "Monthly", amount: "", due_day: "", discount_percent: "", notes: "", session_year: "2025-2026" };
  const [form, setForm] = useState(emptyForm);
  ...
  const save = async () => {
    ...
    const { error } = editing ? await supabase.from("course_fees").update(payload).eq("id", editing.id) : await supabase.from("course_fees").insert(payload);
    if (error) { setErr(error.message); setSaving(false); return; }
    setSaving(false); setShowModal(false); load();
  };
  ...
  // Group: course → subtype → hostel_type → [fee rows]
  const grouped = {};
  fees.forEach(f => {
    const ck = f.course;
    const sk = f.subtype    || "—";
        ...
        <Modal title={editing ? "Edit Fee" : "Add Fee Structure"} onClose={() => setShowModal(false)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              <label style={S.lbl}>Academic Subtype</label>
              {subtypes.length > 0
                ? <select value={form.subtype} onChange={e => setForm(f => ({ ...f, subtype: e.target.value }))} style={S.inp()}><option value="">All Subtypes</option>{subtypes.map(s => <option key={s}>{s}</option>)}</select>
                : <input value={form.subtype} onChange={e => setForm(f => ({ ...f, subtype: e.target.value }))} placeholder="Optional" style={S.inp()} />
              }
            </div>
            <div>
            ...
            <div>
              <label style={S.lbl}>Class Name</label>
              <input value={form.class_name} onChange={e => setForm(f => ({ ...f, class_name: e.target.value }))} placeholder="e.g. Class 6 (optional)" style={S.inp()} />
            </div>
            <div style={{ gridColumn: "1/-1" }}>
              <label style={S.lbl}>Notes</label>
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2} style={{ ...S.inp(), resize: "vertical" }} />
          ...
          <div key={courseName} style={{ marginBottom: 28 }}>
            ...
            {!cGroup ? null : Object.entries(cGroup).map(([subtype, hostelGroups]) => (
              <div key={subtype} style={{ marginBottom: 16, marginLeft: 12 }}>
                ...
                {/* Hostel type cards — one per hostel type */}
                        ...
                        {entries.map(f => (
                          <div key={f.id} style={{ padding: "10px 12px", borderBottom: "1px solid #F3F4F6", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                            <div>
                              ...
                              {f.class_name && <div style={{ fontSize: 11, color: "#9CA3AF" }}>{f.class_name}</div>}
    ...
    <div style={{ minHeight: "100vh", background: "#F8FAFC", fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ background: "white", borderBottom: "1px solid #E5E7EB", padding: "18px 32px", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: "#9CA3AF" }}>Navodaya · Sainik · Foundation · Combined — GNSI</p>
        </div>
        ...
        <div style={{ width: 220, background: "white", borderRight: "1px solid #E5E7EB", padding: "16px 0", flexShrink: 0 }}>
          {NAV_TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              style={{ width: "100%", textAlign: "left", padding: "10px 20px", border: "none", cursor: "pointer", fontFamily: "inherit", background: activeTab === tab.id ? "#F5F3FF" : "transparent", borderRight: activeTab === tab.id ? "3px solid #7C3AED" : "3px solid transparent", color: activeTab === tab.id ? "#7C3AED" : "#374151", fontWeight: activeTab === tab.id ? 600 : 400, fontSize: 13, display: "flex", alignItems: "center", gap: 10, transition: "all .15s" }}>
          ...
          <div style={{ margin: "20px 12px 12px", padding: 14, background: "#F9FAFB", borderRadius: 10, border: "1px solid #F3F4F6" }}>
            <p style={{ margin: "0 0 8px", fontSize: 11, fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: ".05em" }}>Courses</p>
            {Object.entries(COURSE_STRUCTURE).map(([name, c]) => (
            ...
            <p style={{ margin: "12px 0 8px", fontSize: 11, fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: ".05em" }}>Hostel Types</p>
            {HOSTEL_TYPES.map(h => {
              const hc = HOSTEL_COLORS[h];
              return (
                <div key={h} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                  <span style={{ width: 7, height: 7, borderRadius: "50%", background: hc.color, flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: "#374151" }}>{h}</span>
                </div>
              );
            })}
          </div>
...
/*
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SQL — Run in Supabase before deploying
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
alter table course_enrollments
  add column if not exists hostel_type  text,
  add column if not exists session_year text,
  add column if not exists gcc_no       text;
