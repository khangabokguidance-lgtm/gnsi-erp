              ...
              <tr key={u.id} className="adm-row" style={{ borderBottom: i < filtered.length-1 ? '1px solid #F1F5F9' : 'none', transition:'background .1s' }}>
                ...
                <td style={{ padding:'12px 14px' }}>
                  <div style={{ display:'flex', gap:5 }}>
                    ...
                    <button onClick={() => setConfirm({ user: u })} disabled={deleting===u.id} style={{ padding:'5px 12px', borderRadius:6, fontSize:12, cursor:'pointer', border:'1px solid #FECACA', background:'#FEF2F2', color:'#DC2626', fontFamily:'inherit', fontWeight:500 }}>
        ...
        {!selUser ? (
          ...
          <>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14, padding:'12px 16px', background:'white', borderRadius:10, border:'1px solid #E2E8F0', flexWrap:'wrap' }}>
              ...
              <div style={{ marginLeft:'auto', fontSize:12, color:'#94A3B8' }}>{Object.keys(overrides).length} override{Object.keys(overrides).length!==1?'s':''} active</div>
            </div>
...
function AnalyticsSection() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const { data } = await supabase.from('module_access_logs').select('module_key,role,accessed_at').gte('accessed_at', d.toISOString())
  ...
  const dailyMap = {}
  logs.forEach(l => { const day = new Date(l.accessed_at).toLocaleDateString('en-IN',{day:'2-digit',month:'short'}); dailyMap[day]=(dailyMap[day]||0)+1 })
...
function AccessLogsSection() {
  ...
  const fetchLogs = useCallback(async () => {
    setLoading(true)
    let q = supabase.from('module_access_logs').select('id,username,role,module_key,accessed_at', { count:'exact' }).order('accessed_at',{ascending:false}).limit(500)
    if (module !== 'all') q = q.eq('module_key', module)
    if (role   !== 'all') q = q.eq('role', role)
    const { data, count } = await q
    setLogs(data||[]); setTotal(count||0); setPage(0); setLoading(false)
  }, [module, role])
  ...
  return (
    <div>
      <div className="adm-filter-row" style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <span style={{ marginLeft:'auto', fontSize:12, color: total > 500 ? '#D97706' : '#94A3B8' }}>
          {filtered.length} shown{total > 500 ? ` (of ${total} total)` : ''}
        </span>
      </div>
      {loading ? <Spinner /> : (
        <>
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <span style={{ marginLeft:'auto', fontSize:12, color:'#94A3B8' }}>{filtered.length} events</span>
      </div>
