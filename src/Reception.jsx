...
// ─────────────────────────────────────────────────────────────────────────────
//  Tabs: Student 360° | Enquiry | Visitor Book | Gate Pass | Parent Items
//
//  STUDENT FIELD MATCHING (v2)
//  ────────────────────────────
//  buildIndex     : name, gcc_no, admission_no, batch, house, phone, father_name
//  Autocomplete   : shows gcc_no, admission_no, batch, house, father_name, phone
//  Enquiry        : pre-fills student_name, class_interest(batch), phone, parent_name(father_name)
//  Visitor Book   : pre-fills meeting_with context note
//  Gate Pass      : pre-fills student_name, class_name(batch), course
//  Parent Items   : pre-fills student_name, class_name(batch), house, course, hostel_type
//  Student 360°   : uses id, gcc_no, name, admission_no, batch, house, course, session,
//                   hostel_type, gender, dob, phone, status, total_fee, fee_paid
// ─────────────────────────────────────────────────────────────────────────────
...
const TABS    = ['Student 360°', 'Enquiry', 'Visitor Book', 'Gate Pass', 'Parent Items']
...
// Matches COURSES in Students.jsx (excluding 'All')
const COURSES = ['Sainik', 'Navodaya', 'Foundation', 'Combined Course']

// Matches HOSTEL_TYPES in Students.jsx (excluding 'All')
...
// ── default form states ───────────────────────────────────────────────────────
// UPDATED: GP_DEF + PI_DEF now include course / hostel_type
...
// ─────────────────────────────────────────────────────────────────────────────
// UPDATED: buildIndex now includes phone + father_name
    ...
    <div ref={wrapRef} style={{ position: 'relative' }}>
      <div style={{ position: 'relative' }}>
        ...
        <button type="button" tabIndex={-1}
          ...
          style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: '#c0c8d4', lineHeight: 1, padding: '0 2px', opacity: inputRef.current?.value ? 1 : 0, pointerEvents: inputRef.current?.value ? 'auto' : 'none' }}>
          ×
        </button>
      </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {/* UPDATED: now shows father_name + phone in addition to existing fields */}
...
// ─────────────────────────────────────────────────────────────────────────────
// SELECTED STUDENT CHIP — reusable confirmation chip shown after autocomplete pick
// ─────────────────────────────────────────────────────────────────────────────
      ...
      {showCustom && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          <input value={customVal} onChange={e => setCustomVal(e.target.value)}
            placeholder="Type custom item name…" style={{ ...inp, flex: 1 }}
            onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); commit() } }} />
          <button type="button" onClick={commit}
            style={{ padding: '8px 16px', background: '#1e3a5f', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: 13, fontFamily: 'inherit' }}>
            Add
          </button>
        </div>
      )}
      {value && (
        <div style={{ fontSize: 12, color: '#1e3a5f', fontWeight: 700, marginTop: 4 }}>
          ✓ Selected: <span style={{ background: '#dbeafe', padding: '2px 10px', borderRadius: 99 }}>{value}</span>
        </div>
      )}
    </div>
  )
...
body{font-family:Georgia,serif;background:#f0f4f8;display:flex;justify-content:center;padding:36px 16px}
.page{width:680px;background:white;box-shadow:0 4px 32px rgba(0,0,0,.15);overflow:hidden;position:relative}
.wm{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-35deg);font-size:90px;font-weight:900;color:rgba(30,58,95,.04);pointer-events:none;z-index:0;white-space:nowrap}
.hdr{background:#1e3a5f;padding:24px 32px;position:relative;z-index:1;display:flex;justify-content:space-between;align-items:flex-start}
.inst{color:white;font-size:18px;font-weight:700}.sub{color:rgba(255,255,255,.5);font-size:11px;margin-top:3px}
.inv-l{font-size:10px;color:rgba(255,255,255,.5);text-transform:uppercase;letter-spacing:.1em;text-align:right}
.inv-no{font-size:20px;font-weight:800;color:#c9a84c;font-family:'Courier New',monospace;margin-top:2px;text-align:right}
.accent{height:4px;background:linear-gradient(90deg,#7c3aed,#c9a84c)}
.title-row{background:#f8fafc;padding:12px 32px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #e2e8f0}
.title{font-size:14px;font-weight:800;color:#1e3a5f;text-transform:uppercase;letter-spacing:.06em}
.badge{background:#7c3aed;color:white;font-size:10px;font-weight:700;padding:3px 11px;border-radius:99px}
.body{padding:24px 32px;position:relative;z-index:1}
.meta{display:grid;grid-template-columns:1fr 1fr;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;margin-bottom:22px}
.mc{padding:11px 14px;border-right:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0}
.mc:nth-child(even){border-right:none}.mc:nth-last-child(-n+2){border-bottom:none}
.ml{font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.07em;margin-bottom:3px}
.mv{font-size:13px;font-weight:700;color:#1e293b}
.pt{width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;margin-bottom:18px}
.pt thead{background:#1e3a5f}
.pt th{padding:9px 14px;text-align:left;font-size:11px;font-weight:700;color:rgba(255,255,255,.7);text-transform:uppercase;letter-spacing:.05em}
.pt td{padding:11px 14px;font-size:13px;color:#334155;border-bottom:1px solid #f1f5f9}
.status-row{display:flex;justify-content:flex-end;margin-bottom:22px}
.status-box{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px 18px;text-align:right;min-width:160px}
.sl{font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px}
.sv{font-size:14px;font-weight:800;color:#1e293b}
.sig{display:flex;justify-content:space-between;padding-top:28px;border-top:1px solid #e2e8f0}
.sb{text-align:center}
.sl2{width:130px;border-top:1.5px solid #1e3a5f;margin:0 auto 6px}
.st{font-size:11px;color:#64748b}
.ftr{background:#f8fafc;border-top:1px solid #e2e8f0;padding:11px 32px;display:flex;justify-content:space-between;font-size:10px;color:#94a3b8}
@media print{body{background:white;padding:0}.page{box-shadow:none;width:100%}.hdr,.accent,.badge,.pt thead{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
</style></head><body>
<div class="page">
<div class="wm">GNSI</div>
<div class="hdr">
  <div><div class="inst">Guidance Navodaya &amp; Sainik Institute</div><div class="sub">Khangabok, Thoubal, Manipur — 795128</div></div>
  <div><div class="inv-l">Invoice No.</div><div class="inv-no">${invoiceNo}</div></div>
</div>
<div class="accent"></div>
<div class="title-row"><div class="title">Parent Item Receipt</div><div class="badge">Parent Items</div></div>
<div class="body">
  <div class="meta">
    <div class="mc"><div class="ml">Student Name</div><div class="mv">${item.student_name || '—'}</div></div>
    <div class="mc"><div class="ml">Class / Batch</div><div class="mv">${item.class_name || '—'}</div></div>
    <div class="mc"><div class="ml">Course</div><div class="mv">${item.course || '—'}</div></div>
    <div class="mc"><div class="ml">Hostel Type</div><div class="mv">${item.hostel_type || '—'}</div></div>
    <div class="mc"><div class="ml">Parent Name</div><div class="mv">${item.parent_name || '—'}</div></div>
    <div class="mc"><div class="ml">House / Block</div><div class="mv">${item.house || '—'}</div></div>
    <div class="mc"><div class="ml">Date Received</div><div class="mv">${fmtDate(item.received_date)}</div></div>
    <div class="mc"><div class="ml">Received By</div><div class="mv">${item.received_by || '—'}</div></div>
  </div>
  <table class="pt">
    <thead><tr><th style="width:32px">#</th><th>Item Description</th><th>Quantity</th><th>Status</th></tr></thead>
    <tbody>
      <tr><td>1</td><td style="font-weight:700">${item.item_name}</td><td>${item.quantity || '1'}</td><td style="font-weight:700;color:#7c3aed">${item.status}</td></tr>
    </tbody>
  </table>
  <div class="status-row">
    <div class="status-box">
      <div class="sl">Current Status</div>
      <div class="sv">${item.status}</div>
      ${item.remarks ? `<div style="font-size:11px;color:#64748b;margin-top:4px">${item.remarks}</div>` : ''}
    </div>
  </div>
  <div class="sig">
    <div class="sb"><div class="sl2"></div><div class="st">Parent / Guardian</div></div>
    <div class="sb"><div class="sl2"></div><div class="st">Received By (Staff)</div></div>
    <div class="sb"><div class="sl2"></div><div class="st">Warden / HOD</div></div>
  </div>
</div>
<div class="ftr"><span>GNSI · Parent Item Invoice · ${item.student_name}</span><span>Printed: ${d}</span></div>
</div></body></html>`
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Georgia,serif;background:#fff;padding:32px;color:#1e293b}
...
.title{font-size:20px;font-weight:800;color:#1e3a5f;margin-bottom:16px;text-transform:uppercase;letter-spacing:.08em}
.grid{display:grid;grid-template-columns:1fr 1fr;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;margin-bottom:20px}
.cell{padding:11px 14px;border-right:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0}
.cell:nth-child(even){border-right:none}.cell:nth-last-child(-n+2){border-bottom:none}
.cl{font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.07em;margin-bottom:3px}
.cv{font-size:13px;font-weight:700;color:#1e293b}
.sig{display:flex;justify-content:space-between;margin-top:32px;padding-top:20px;border-top:1px solid #e2e8f0}
.sb{text-align:center}.sl{width:140px;border-top:1.5px solid #1e3a5f;margin:0 auto 6px}.st{font-size:11px;color:#64748b}
.ftr{margin-top:24px;text-align:center;font-size:10px;color:#94a3b8}
@media print{body{padding:16px}}</style></head><body>
<div class="hdr"><div><div class="inst">Guidance Navodaya &amp; Sainik Institute</div><div class="sub">Khangabok, Thoubal, Manipur — 795128</div></div>
<div style="text-align:right"><div style="font-size:10px;color:#94a3b8">Printed</div><div style="font-weight:700;font-size:13px">${d}</div></div></div>
...
<div class="grid">
  ...
  <div class="cell"><div class="cl">Course</div><div class="cv">${item.course || '—'}</div></div>
  <div class="cell"><div class="cl">Reason</div><div class="cv">${item.reason}</div></div>
  <div class="cell"><div class="cl">Exit Date &amp; Time</div><div class="cv">${fmtDate(item.exit_date)} ${item.exit_time ? '· ' + item.exit_time : ''}</div></div>
  <div class="cell"><div class="cl">Approved By</div><div class="cv">${item.approved_by || '—'}</div></div>
  <div class="cell"><div class="cl">Parent Informed</div><div class="cv">${item.parent_informed}</div></div>
  <div class="cell"><div class="cl">Status</div><div class="cv">${item.status}</div></div>
  <div class="cell" style="grid-column:1/-1"><div class="cl">Remarks</div><div class="cv">${item.remarks || '—'}</div></div>
</div>
<div class="sig">
  <div class="sb"><div class="sl"></div><div class="st">Student Signature</div></div>
  <div class="sb"><div class="sl"></div><div class="st">Class Teacher</div></div>
  <div class="sb"><div class="sl"></div><div class="st">Principal / Warden</div></div>
</div>
<div class="ftr">GNSI · Gate Pass · Computer generated · ${d}</div>
</body></html>`
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Georgia,serif;background:#f0f4f8;display:flex;justify-content:center;padding:40px}
.badge{width:320px;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.15)}
.top{background:#1e3a5f;padding:18px;text-align:center}
.inst{color:white;font-size:13px;font-weight:700}.sub{color:rgba(255,255,255,.5);font-size:10px;margin-top:2px}
.bl{background:#c9a84c;color:#1e3a5f;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.12em;padding:5px 0;text-align:center}
.body{padding:18px}
.av{width:56px;height:56px;border-radius:50%;background:#1e3a5f;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:800;color:#c9a84c;margin:0 auto 12px}
.name{text-align:center;font-size:17px;font-weight:800;color:#1e293b;margin-bottom:3px}
.purpose{text-align:center;font-size:12px;color:#64748b;margin-bottom:14px}
.row{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:12px}
.rk{color:#94a3b8;font-weight:600}.rv{color:#1e293b;font-weight:700}
.ftr{background:#f8fafc;padding:9px;text-align:center;font-size:10px;color:#94a3b8;border-top:1px solid #e2e8f0}
@media print{body{background:white;padding:0}.badge{box-shadow:none}}</style></head><body>
<div class="badge">
<div class="top"><div class="inst">Guidance Navodaya &amp; Sainik Institute</div><div class="sub">Khangabok · Thoubal · Manipur</div></div>
<div class="bl">Visitor Pass</div>
<div class="body">
<div class="av">${(item.visitor_name || 'V')[0].toUpperCase()}</div>
<div class="name">${item.visitor_name}</div><div class="purpose">${item.purpose}</div>
<div class="row"><span class="rk">Meeting With</span><span class="rv">${item.meeting_with || '—'}</span></div>
<div class="row"><span class="rk">Visit Date</span><span class="rv">${fmtDate(item.visit_date)}</span></div>
<div class="row"><span class="rk">In Time</span><span class="rv">${item.in_time || '—'}</span></div>
<div class="row"><span class="rk">Phone</span><span class="rv">${item.phone || '—'}</span></div>
<div class="row"><span class="rk">ID Proof</span><span class="rv">${item.id_proof || '—'}</span></div>
</div>
<div class="ftr">Computer generated · ${d}</div>
</div></body></html>`
...
function HouseWiseGrid({ parentItems, onStatusChange }) {
  ...
  const filtered = useMemo(() => {
    if (statusFilter === 'All') return parentItems
    return parentItems.filter(p => p.status === statusFilter)
  }, [parentItems, statusFilter])

  const grouped = useMemo(() => {
    const g = {}
    filtered.forEach(item => {
      const house = item.house || 'Day Scholar / Unassigned'
      if (!g[house]) g[house] = []
      g[house].push(item)
    })
    return g
  ...
  return (
    <div>
      ...
      {houseNames.length === 0 && (
        <div style={{ padding: 40, textAlign: 'center', color: '#94a3b8', fontSize: 14 }}>No records match this filter.</div>
      )}
          ...
          return (
            <div key={house} style={{ background: 'white', borderRadius: 14, overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,.06)' }}>
              <div style={{ background: hColor, padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: 'white' }}>🏠 {house}</div>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,.6)', marginTop: 2 }}>{hItems.length} item{hItems.length !== 1 ? 's' : ''}</div>
                </div>
                  ...
                  <div key={item.id || i} style={{ padding: '11px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: '#1e293b', display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        {item.item_name}
                        {item.quantity && item.quantity !== '1' && <span style={{ fontSize: 11, color: '#64748b', fontWeight: 400 }}>× {item.quantity}</span>}
                      </div>
...
function SectionHead({ icon, title, color, badge, badgeColor }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
      ...
      {badge !== undefined && (
        <span style={{ fontSize: 11, fontWeight: 700, background: (badgeColor || color) + '18', color: badgeColor || color, padding: '2px 9px', borderRadius: 99 }}>{badge}</span>
      )}
    </div>
  )
}
...
function Student360({ students }) {
  ...
  const [resetKey360, setResetKey360] = useState(0)
          ...
          <div>
            <div style={{ background: 'linear-gradient(135deg,#1e3a5f 0%,#0f2340 100%)', borderRadius: 14, padding: '20px 26px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#c9a84c', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 900, color: '#1e3a5f', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <button onClick={() => { setSel(null); setProf(null); setResetKey360(k => k + 1) }} style={{ padding: '7px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,.25)', background: 'transparent', color: 'white', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>← Change</button>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              ...
              <div style={card}>
                ...
                {hostel
                  ? <InfoGrid rows={[
                      ['Room No.',   hostel.hostel_rooms?.room_no],
                      ['Floor',      hostel.hostel_rooms?.floor],
                      ['Room Type',  hostel.hostel_rooms?.room_type],
                      ['Bed No.',    hostel.bed_no],
                    ...
                    <tbody>
                      {parentItems.map(p => (
                        <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          ...
                          <td style={tdS}>{p.course || '—'}</td>
                          <td style={tdS}>{p.hostel_type || '—'}</td>
...
function FormInput({ field, value, onChange, type = 'text', placeholder, required }) {
  return (
    <input type={type} placeholder={placeholder} required={required} style={inp}
...
function SaveBtn({ label, saving }) {
  return (
    <button type="submit" disabled={saving}
      style={{ marginTop: 16, background: '#1e3a5f', color: '#fff', padding: '10px 22px', borderRadius: 9, border: 'none', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
      {saving ? 'Saving…' : label}
    </button>
  )
}
...
export default function ReceptionPage() {
  ...
  // ── UPDATED onSelect callbacks — all fields now matched ───────────────────

  // Enquiry: name + batch→class_interest + phone + father_name→parent_name
  const onSelectEnquiry = useCallback(s => {
    if (s) {
      ...
      setEnquiryForm(f => ({
        ...f,
        student_name:   s.name,
        class_interest: s.batch       || f.class_interest,
        phone:          s.phone       || f.phone,
        parent_name:    s.father_name || f.parent_name,
      }))
    } else {
      setEnquiryStudent(null)
      setEnquiryForm(f => ({ ...f, student_name: '', parent_name: '' }))
    }
  }, [])

  // Visitor: context hint in meeting_with only
  const onSelectVisitor = useCallback(s => {
    if (s) {
      setVisitorStudent(s)
      setVisitorForm(f => ({ ...f, meeting_with: f.meeting_with || `Re: ${s.name}${s.batch ? ' (' + s.batch + ')' : ''}` }))
    } else {
      setVisitorStudent(null)
    }
  }, [])

  // Gate Pass: name + batch→class_name + course
  const onSelectGP = useCallback(s => {
    if (s) {
      ...
      setGpForm(f => ({
        ...f,
        student_name: s.name,
        class_name:   s.batch  || f.class_name,
        course:       s.course || f.course,
      }))
    } else {
      setGpStudent(null)
      setGpForm(f => ({ ...f, student_name: '', class_name: '', course: '' }))
    }
  }, [])

  // Parent Items: name + batch→class_name + house + course + hostel_type
  const onSelectPI = useCallback(s => {
    if (s) {
      ...
      setPiForm(f => ({
        ...
        course:       s.course       || f.course,
        hostel_type:  s.hostel_type  || f.hostel_type,
      }))
    } else {
      setPiStudent(null)
      setPiForm(f => ({ ...f, student_name: '', class_name: '', house: '', course: '', hostel_type: '' }))
    }
  }, [])
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'system-ui,sans-serif', background: '#f0f4f8', minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: '#64748b', fontSize: 13, marginTop: 4 }}>Enquiries · Visitors · Gate Passes · Parent Items · Student 360°</p>
      </div>
      ...
      {/* Search bar (non-360 tabs) */}
      {activeTab !== 'Student 360°' && (
        <div style={{ ...card, marginBottom: 16 }}>
          ...
          <div style={{ ...card, marginBottom: 16 }}>
            ...
            <form onSubmit={e => {
              ...
              handleInsert('reception_enquiries', enquiryForm, () => {
                setEnquiryForm({ ...ENQ_DEF, enquiry_date: today() })
                setEnquiryStudent(null); setEnquiryResetKey(k => k + 1)
              })
            }}>
              <div style={grid2}>
                <div style={span2}>
          ...
          <div style={{ ...card, marginBottom: 16 }}>
            ...
            <form onSubmit={e => {
              ...
              handleInsert('reception_visitors', visitorForm, () => {
                setVisitorForm({ ...VIS_DEF, visit_date: today() })
                setVisitorStudent(null); setVisitorResetKey(k => k + 1)
              })
            }}>
              <div style={grid2}>
                <div style={span2}>
      ...
      {activeTab === 'Gate Pass' && (
        <>
          <div style={{ ...card, marginBottom: 16 }}>
            ...
            <form onSubmit={e => {
              e.preventDefault()
              handleInsert('reception_gatepasses', gpForm, () => {
                setGpForm({ ...GP_DEF, exit_date: today() })
                setGpStudent(null); setGpResetKey(k => k + 1)
              })
            }}>
              <div style={grid2}>
                <div style={span2}>
                ...
                <FormField label="Class">
                  <FormSelect field="class_name" value={gpForm.class_name} onChange={set_gp} options={CLASS_OPTIONS} placeholder="Select class…" />
                </FormField>
                <FormField label="Course">
                  <FormSelect field="course" value={gpForm.course} onChange={set_gp} options={COURSES} placeholder="Select course…" />
                </FormField>
                <FormField label="Reason *">
                  <FormSelect field="reason" value={gpForm.reason} onChange={set_gp} options={GP_REASON_OPTIONS} placeholder="Select reason…" />
                </FormField>
                ...
                <FormField label="Approved By">
                  <FormSelect field="approved_by" value={gpForm.approved_by} onChange={set_gp} options={APPROVED_BY_OPTIONS} placeholder="Select approver…" />
                </FormField>
                ...
                <FormField label="Status">
                  <FormSelect field="status" value={gpForm.status} onChange={set_gp} options={['Issued','Exited','Returned']} />
                </FormField>
        ...
        <>
          <div style={{ ...card, marginBottom: 16 }}>
            ...
            <form onSubmit={e => {
              ...
              handleInsert('reception_parent_items', piForm, () => {
                setPiForm({ ...PI_DEF, received_date: today() })
                setPiStudent(null); setPiResetKey(k => k + 1)
              })
            }}>
              <div style={grid2}>
                <div style={span2}>
                ...
                <FormField label="Class / Batch">
                  <FormSelect field="class_name" value={piForm.class_name} onChange={set_pi} options={CLASS_OPTIONS} placeholder="Select class…" />
                </FormField>
                <FormField label="Course">
                  <FormSelect field="course" value={piForm.course} onChange={set_pi} options={COURSES} placeholder="Select course…" />
                </FormField>
                <FormField label="Hostel Type">
                  <FormSelect field="hostel_type" value={piForm.hostel_type} onChange={set_pi} options={HOSTEL_TYPES} placeholder="Select hostel type…" />
                </FormField>
                <FormField label="House / Hostel Block">
                  <FormSelect field="house" value={piForm.house} onChange={set_pi} options={HOUSE_OPTIONS} placeholder="Select house / block…" />
                </FormField>
                ...
                <FormField label="Received By">
                  <FormSelect field="received_by" value={piForm.received_by} onChange={set_pi} options={RECEIVED_BY_OPTIONS} placeholder="Select staff…" />
                </FormField>
                <FormField label="Status">
                  <FormSelect field="status" value={piForm.status} onChange={set_pi} options={['Pending','Delivered','Returned']} />
                </FormField>
